...
)  # pylint: disable=W0611,C0411,C0412
...
def load_model():
    ...
    shared.opts.onchange(
        "sd_model_refiner",
        wrap_queued_call(lambda: modules.sd_models.reload_model_weights(op="refiner")),
        call=False,
    )
    shared.opts.onchange(
        "sd_model_dict",
        wrap_queued_call(lambda: modules.sd_models.reload_model_weights(op="dict")),
        call=False,
    )
    shared.opts.onchange(
        "sd_backend",
        wrap_queued_call(lambda: modules.sd_models.change_backend()),
        call=False,
    )
...
def start_ui():
    ...
    if cmd_opts.auth:
        gradio_auth_creds += [
            x.strip()
            for x in cmd_opts.auth.strip('"').replace("\n", "").split(",")
            if x.strip()
        ]
    ...
    (
        ...
        local_url,
        share_url,
    ) = shared.demo.launch(  # app is FastAPI(Starlette) instance
    ...
    time_setup = [
        f"{k}:{round(v,3)}s"
        for (k, v) in modules.scripts.time_setup.items()
        if v > 0.005
    ]
    shared.log.debug(f"Scripts setup: {time_setup}")
    time_component = [
        f"{k}:{round(v,3)}s"
        for (k, v) in modules.scripts.time_component.items()
        if v > 0.005
    ]
