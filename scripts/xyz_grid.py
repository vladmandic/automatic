...
def apply_prompt(p, x, xs):
    if xs[0] not in p.prompt and xs[0] not in p.negative_prompt:
        shared.log.warning(
            f"XYZ grid: prompt S/R did not find {xs[0]} in prompt or negative prompt."
        )
    else:
...
def apply_dict(p, x, xs):
    ...
    if info_dict is None or info_ckpt is None:
        ...
        shared.opts.sd_model_dict = (
            info_dict.name
        )  # this will trigger reload_model_weights via onchange handler
...
axis_options = [
    ...
    AxisOption(
        ...
        fmt=format_value,
        cost=1.0,
        choices=lambda: sorted(sd_models.checkpoints_list),
    ),
    AxisOption(
        "VAE",
        str,
    ...
    AxisOption(
        "Prompt order", str_permutations, apply_order, fmt=format_value_join_list
    ),
    ...
    AxisOptionImg2Img(
        "Image Mask Weight", float, apply_field("inpainting_mask_weight")
    ),
    ...
    AxisOption(
        "SecondPass Sampler",
        str,
        apply_latent_sampler,
        fmt=format_value,
        confirm=confirm_samplers,
        choices=lambda: [x.name for x in sd_samplers.samplers],
    ),
    AxisOption(
        "SecondPass Denoising Strength", float, apply_field("denoising_strength")
    ),
    ...
    AxisOption(
        "SecondPass Guidance Rescale", float, apply_field("diffusers_guidance_rescale")
    ),
...
def draw_xyz_grid(
    ...
    if not processed_result:
        shared.log.error(
            "XYZ grid: Processing could not begin, you may need to refresh the tab or restart the service"
        )
        return Processed(p, [])
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row(variant="compact", elem_id="axis_options"):
            ...
            no_fixed_seeds = gr.Checkbox(
                label="Keep random for seeds",
                value=False,
                elem_id=self.elem_id("no_fixed_seeds"),
            )
            no_grid = gr.Checkbox(
                label="Do not create grid",
                value=False,
        ...
        with gr.Row(variant="compact", elem_id="swap_axes"):
            swap_xy_axes_button = gr.Button(
                value="Swap X/Y",
                elem_id="xy_grid_swap_axes_button",
                variant="secondary",
            )
            swap_yz_axes_button = gr.Button(
                value="Swap Y/Z",
                elem_id="yz_grid_swap_axes_button",
                variant="secondary",
            )
            swap_xz_axes_button = gr.Button(
                value="Swap X/Z",
                elem_id="xz_grid_swap_axes_button",
                variant="secondary",
            )
    ...
    def run(
        ...
        shared.log.info(
            f"XYZ grid: images={len(xs)*len(ys)*len(zs)*image_cell_count} grid={len(zs)} {len(xs)}x{len(ys)} cells={len(zs)} steps={total_steps}"
        )
