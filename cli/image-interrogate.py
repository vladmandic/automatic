    ...
    if "caption" in res:
        ...
        for word in caption.split(" "):
            if word not in exclude:
                stats["captions"][word] = (
                    stats["captions"][word] + 1 if word in stats["captions"] else 1
                )
    else:
    ...
    if "caption" in res:
        ...
        for word in keywords.items():
            stats["keywords"][word[0]] = (
                stats["keywords"][word[0]] + 1 if word[0] in stats["keywords"] else 1
            )
        ...
        if os.path.exists(arg):
            if os.path.isfile(arg):
                ...
                for root, _dirs, files in os.walk(arg):
                    for f in files:
                        _caption, _keywords, _style = await interrogate(
                            os.path.join(root, f)
                        )
            else:
