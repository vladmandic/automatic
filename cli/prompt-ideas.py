...
if __name__ == "__main__":  # create & train test embedding when used from cli
    ...
    parser.add_argument(
        ...
        default=0.9,
        required=False,
        help="higher temperature produces more diverse results with a higher risk of less coherent text, default: %(default)s",
    )
    parser.add_argument(
        "--top",
        type=int,
        default=8,
        required=False,
        help="number of tokens to sample from at each step, default: %(default)s",
    )
    parser.add_argument(
        "--penalty",
        type=float,
        default=1.2,
        required=False,
        help="penalty value for each repetition of a token, default: %(default)s",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0.6,
        required=False,
        help="penalty alpha value, default: %(default)s",
    )
    parser.add_argument(
        "--num",
        type=int,
        default=10,
        required=False,
        help="number of results to generate, default: %(default)s",
    )
    parser.add_argument(
        "--length",
        type=int,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="print extra debug information, default: %(default)s",
    )
