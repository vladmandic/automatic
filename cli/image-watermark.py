...
def get_exif(image):
    ...
    try:
        ...
        for k, v in exif.items():
            key = list(vars(piexif.ExifIFD).keys())[
                list(vars(piexif.ExifIFD).values()).index(k)
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wm", type=str, required=False, default="sdnext", help="watermark string"
    )
    parser.add_argument(
        "--strip", default=False, action="store_true", help="strip existing exif data"
    )
    parser.add_argument(
        "--verify",
        default=False,
        action="store_true",
        help="verify watermark during write",
    )
    parser.add_argument(
        "--length", type=int, default=32, help="watermark length in bits"
    )
    parser.add_argument(
        "--output",
        type=str,
        required=False,
        default="",
        help="folder to store images, default is overwrite in-place",
    )
