...
def grid(data):
    if len(data.image) > 1:
        ...
        short = data.info.prompt[
            : min(len(data.info.prompt), 96)
        ]  # limit prompt part of filename to 96 chars
        name = "{seed:0>9} {short}".format(
            short=short, seed=data.info.all_seeds[0]
        ...
        image.save(
            f, "JPEG", exif=exif(data.info, None, "grid"), optimize=True, quality=70
        )
        return image
    return data.image
...
def exif(info, i=None, op="generate"):
    seed = (
        [info.all_seeds[i]]
        if len(info.all_seeds) > 0 and i is not None
        else info.all_seeds
    )  # always returns list
    seed = ", ".join([str(x) for x in seed])  # int list to str list to single str
    template = "{prompt} | negative {negative_prompt} | seed {s} | steps {steps} | cfgscale {cfg_scale} | sampler {sampler_name} | batch {batch_size} | timestamp {job_timestamp} | model {model} | vae {vae}".format(
        s=seed,
        model=sd.options["sd_model_checkpoint"],
        vae=sd.options["sd_vae"],
        **info
    )  # pylint: disable=consider-using-f-string
    if op == "upscale":
        template += " | faces gfpgan" if sd.upscale.gfpgan_visibility > 0 else ""
        template += (
            " | faces codeformer" if sd.upscale.codeformer_visibility > 0 else ""
        )
        template += (
            " | upscale {resize}x {upscaler}".format(
                resize=sd.upscale.upscaling_resize, upscaler=sd.upscale.upscaler_1
            )
            if sd.upscale.upscaler_1 != "None"
            else ""
        )  # pylint: disable=consider-using-f-string
        template += (
            " | upscale {resize}x {upscaler}".format(
                resize=sd.upscale.upscaling_resize, upscaler=sd.upscale.upscaler_2
            )
            if sd.upscale.upscaler_2 != "None"
            else ""
        )  # pylint: disable=consider-using-f-string
    if op == "grid":
        template += " | grid {num}".format(
            num=sd.generate.batch_size * sd.generate.n_iter
        )  # pylint: disable=consider-using-f-string
...
def prompt(params):  # generate dynamic prompt or use one if provided
    sd.generate.prompt = (
        params.prompt if params.prompt != "dynamic" else randomize(random.prompts)
    )
    sd.generate.negative_prompt = (
        params.negative if params.negative != "dynamic" else randomize(random.negative)
    )
    embedding = (
        params.embedding
        if params.embedding != "random"
        else randomize(random.embeddings)
    )
...
def sampler(params, options):  # find sampler
    if params.sampler == "random":
        ...
        if len(found) == 0:
            log.error(
                {
                    "sampler error": sd.generate.sampler_name,
                    "available": options.samplers,
                }
            )
            exit()
...
async def generate(
    ...
    images = data["images"]
    short = info.prompt[
        : min(len(info.prompt), 96)
    ]  # limit prompt part of filename to 64 chars
    for i in range(len(images)):
        b64s.append(images[i])
        images[i] = Image.open(io.BytesIO(base64.b64decode(images[i].split(",", 1)[0])))
        name = "{seed:0>9} {short}".format(
            short=short, seed=info.all_seeds[i]
        )  # pylint: disable=consider-using-f-string
        name = safestring(name) + ".jpg"
...
async def upscale(data):
    ...
    for i in range(len(data.image)):
        ...
        image.save(
            f, "JPEG", exif=exif(data.info, i, "upscale"), optimize=True, quality=70
        )
    return data
...
async def init():
    ...
    found = (
        sd.options.sd_model_checkpoint
        if sd.options.sd_model_checkpoint in options.models
        else None
    )
    if found is None:
        found = [
            i for i in options.models if i.startswith(sd.options.sd_model_checkpoint)
        ]
    if len(found) == 0:
        log.error(
            {
                "model error": sd.generate.sd_model_checkpoint,
    ...
    log.info(
        {
            "target models": {
                "diffuser": options.options["sd_model_checkpoint"],
                "vae": options.options["sd_vae"],
            }
        }
    )
    ...
    pathlib.Path(os.path.join(sd.paths.root, sd.paths.generate)).mkdir(
        parents=True, exist_ok=True
    )
    pathlib.Path(os.path.join(sd.paths.root, sd.paths.upscale)).mkdir(
        parents=True, exist_ok=True
    )
    pathlib.Path(os.path.join(sd.paths.root, sd.paths.grid)).mkdir(
        parents=True, exist_ok=True
    )
    return options
...
def args():  # parse cmd arguments
    ...
    parser.add_argument(
        ...
        default="random.json",
        required=False,
        help="prompt file with randomized sections",
    )
    parser.add_argument(
        "--max",
        type=int,
    ...
    parser.add_argument(
        "--prompt", type=str, default="dynamic", required=False, help="prompt"
    )
    parser.add_argument(
        "--negative",
        type=str,
    ...
    parser.add_argument(
        ...
        default="random",
        required=False,
        help="artist style, used to guide dynamic prompt when prompt is not provided",
    )
    parser.add_argument(
        "--embedding",
        type=str,
        default="random",
        required=False,
        help="use embedding, used to guide dynamic prompt when prompt is not provided",
    )
    parser.add_argument(
        "--style",
        type=str,
        default="random",
        required=False,
        help="image style, used to guide dynamic prompt when prompt is not provided",
    )
    parser.add_argument(
        "--suffix",
        type=str,
        default="random",
        required=False,
        help="style suffix, used to guide dynamic prompt when prompt is not provided",
    )
    parser.add_argument(
        "--place",
        type=str,
        default="random",
        required=False,
        help="place locator, used to guide dynamic prompt when prompt is not provided",
    )
    parser.add_argument(
        "--faces",
        default=False,
        action="store_true",
        help="restore faces during upscaling",
    )
    parser.add_argument(
        "--steps", type=int, default=0, required=False, help="number of steps"
    )
    parser.add_argument(
        "--batch",
        type=int,
        default=0,
        required=False,
        help="batch size, limited by gpu vram",
    )
    parser.add_argument(
        "--n", type=int, default=0, required=False, help="number of iterations"
    )
    parser.add_argument(
        "--cfg",
        type=int,
    ...
    parser.add_argument(
        "--sampler", type=str, default="random", required=False, help="sampler"
    )
    parser.add_argument(
        "--seed", type=int, default=0, required=False, help="seed, default is random"
    )
    parser.add_argument(
        "--upscale",
        type=int,
        default=0,
        required=False,
        help="upscale factor, disabled if 0",
    )
    parser.add_argument(
        "--model", type=str, default="", required=False, help="diffusion model"
    )
    parser.add_argument("--vae", type=str, default="", required=False, help="vae model")
    parser.add_argument(
        "--path", type=str, default="", required=False, help="output path"
    )
    ...
    parser.add_argument(
        "--beautify", default=False, action="store_true", help="beautify prompt"
    )
    parser.add_argument(
        "--prompts",
        default=False,
        action="store_true",
        help="print dynamic prompt templates",
    )
    parser.add_argument(
        "--debug",
        default=False,
    ...
    if params.prompt == "dynamic":
        ...
        if os.path.isfile(params.random):
            ...
            try:
                with open(
                    os.path.join(home, params.random), "r", encoding="utf-8"
                ) as f:
                    data = json.load(f)
                    random = Map(data)
                    log.debug({"random template": sd})
    ...
    sd.generate.restore_faces = (
        params.faces if params.faces is not None else sd.generate.restore_faces
    )
    sd.generate.seed = params.seed if params.seed > 0 else sd.generate.seed
    sd.generate.sampler_name = (
        params.sampler if params.sampler != "random" else sd.generate.sampler_name
    )
    sd.generate.batch_size = (
        params.batch if params.batch > 0 else sd.generate.batch_size
    )
    ...
    sd.upscale.upscaling_resize = (
        params.upscale if params.upscale > 0 else sd.upscale.upscaling_resize
    )
    sd.upscale.codeformer_visibility = (
        1 if params.faces else sd.upscale.codeformer_visibility
    )
    sd.options.sd_vae = params.vae if params.vae != "" else sd.options.sd_vae
    sd.options.sd_model_checkpoint = (
        params.model if params.model != "" else sd.options.sd_model_checkpoint
    )
...
async def main():
    ...
    while True:
        ...
        log.info(
            {
                "iteration": iteration,
                "batch": sd.generate.batch_size,
                "n": sd.generate.n_iter,
                "total": sd.generate.n_iter * sd.generate.batch_size,
            }
        )
        ...
        stats.wall += t2 - t0
        its = (
            sd.generate.steps / ((t1 - t0) / len(data.image))
            if len(data.image) > 0
            else 0
        )
        ...
        log.info(
            {
                ...
                "max": params.max,
                "progress": round(100 * stats.images / params.max, 1),
            }
        )
