...
options = Map(
    {
        ...
        "prompts": [
            (
                "photo citiscape",
                "cityscape during night, photorealistic, high detailed, sharp focus, depth of field, 4k",
            ),
            (
                "photo car",
                "photo of a sports car, high detailed, sharp focus, dslr, cinematic lighting, realistic",
            ),
            (
                "photo woman",
                "portrait photo of beautiful woman, high detailed, dslr, 35mm",
            ),
            (
                "photo naked",
                "full body photo of beautiful sexy naked woman, high detailed, dslr, 35mm",
            ),
            (
                "photo taylor",
                "portrait photo of beautiful woman taylor swift, high detailed, sharp focus, depth of field, dslr, 35mm <lora:taylor-swift:1>",
            ),
            (
                "photo ti-mia",
                'portrait photo of beautiful woman "ti-mia", naked, high detailed, dslr, 35mm',
            ),
            (
                "photo ti-vlado",
                'portrait photo of man "ti-vlado", high detailed, dslr, 35mm',
            ),
            (
                "photo lora-vlado",
                "portrait photo of man vlado, high detailed, dslr, 35mm <lora:vlado-original:1>",
            ),
            (
                "wlop",
                "a stunning portrait of sexy teen girl in a wet t-shirt, vivid color palette, digital painting, octane render, highly detailed, particles, light effect, volumetric lighting, art by wlop",
            ),
            (
                "greg rutkowski",
                "beautiful woman, high detailed, sharp focus, depth of field, 4k, art by greg rutkowski",
            ),
            (
                "carne griffiths",
                "beautiful woman taylor swift, high detailed, sharp focus, depth of field, art by carne griffiths <lora:taylor-swift:1>",
            ),
            (
                "carne griffiths",
                "man vlado, high detailed, sharp focus, depth of field, art by carne griffiths <lora:vlado-full:1>",
            ),
        ],
        # save format
        "format": ".jpg",
...
async def preview_models(params):
    ...
    log.info(
        {
            "total jobs": len(models) * options.generate.batch_size,
            "per-model": options.generate.batch_size,
        }
    )
    log.info(json.dumps(options, indent=2))
    for model in models:
        if (
            preview_exists(opt["ckpt_dir"], model) and len(params.input) == 0
        ):  # if model preview exists and not manually included
            ...
            log.info(
                {
                    ...
                    "label": label,
                    "prompt": options.generate.prompt,
                }
            )
...
async def lora(params):
    ...
    for model in models:
        if (
            preview_exists("", model) and len(params.input) == 0
        ):  # if model preview exists and not manually included
...
async def hypernetwork(params):
    ...
    for model in models:
        if (
            preview_exists(folder, model) and len(params.input) == 0
        ):  # if model preview exists and not manually included
        ...
        options.generate.prompt = options.prompt.replace(
            "<keyword>", options.hypernetwork.keyword
        )
        options.generate.prompt = options.generate.prompt.replace("<embedding>", "")
        options.generate.prompt = (
            f" <hypernet:{model}:{options.hypernetwork.strength}> "
            + options.generate.prompt
        )
        log.info(
            {
                "hypernetwork generating": model,
                "keyword": keyword,
                "prompt": options.generate.prompt,
            }
        )
        ...
        log.info(
            {
                "hypernetwork preview created": model,
                "image": fn,
                "images": len(images),
                "grid": [image.width, image.height],
                "time": round(t, 2),
...
async def embedding(params):
    ...
    for model in models:
        ...
        log.info(
            {
                "embeding preview created": model,
                "image": fn,
                "images": len(images),
                "grid": [image.width, image.height],
                "time": round(t, 2),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="model used to create extra network previews",
    )
    parser.add_argument(
        "--exclude",
        default=["sd-v20", "sd-v21", "inpainting", "pix2pix"],
        help="exclude models with keywords",
    )
    parser.add_argument(
        "--debug",
        default=False,
