...
def get_latents(local_vae, images, weight_dtype):
    ...
    with torch.no_grad():
        latents = (
            local_vae.encode(img_tensors).latent_dist.sample().float().to("cpu").numpy()
        )
    return latents
...
def create_vae_latents(local_params):
    ...
    bucket_manager = train_util.BucketManager(
        args.noupscale, max_reso, args.min, args.max, args.steps
    )
    ...
    for data_entry in tqdm(data, smoothing=0.0):
        ...
        image_key = os.path.join(
            os.path.basename(pathlib.Path(image_path).parent),
            pathlib.Path(image_path).stem,
        )
    ...
    for i, reso in enumerate(bucket_manager.resos):
        ...
        if count > 0:
            console.log(
                f"vae latents bucket: {i+1}/{len(bucket_manager.resos)} resolution: {reso} images: {count} mean-ar-error: {np.mean(img_ar_errors)}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json", type=str, required=True, help="metadata file to input"
    )
    parser.add_argument(
        "--vae", type=str, required=True, help="model name or path to encode latents"
    )
    ...
    parser.add_argument(
        "--min", type=int, default=256, help="minimum resolution for buckets"
    )
    parser.add_argument(
        "--max", type=int, default=1024, help="maximum resolution for buckets"
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=64,
        help="steps of resolution for buckets, divisible by 8",
    )
    parser.add_argument(
        "--noupscale",
        action="store_true",
