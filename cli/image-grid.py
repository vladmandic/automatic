...
def grid(
    ...
    for i, img in enumerate(images):  # pylint: disable=redefined-outer-name
        ...
        if labels is not None and len(images) == len(labels):
            ...
            ctx.text(
                (x + 1 + round(w / 200), y + 1 + round(w / 200)),
                label,
                font=font,
                fill=(0, 0, 0),
            )
    ...
    log.info(
        {"grid": {"images": len(images), "rows": rows, "cols": cols, "cell": [w, h]}}
    )
    return image
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--square", default=False, action="store_true", help="create square grid"
    )
    parser.add_argument(
        "--horizontal",
        default=False,
        action="store_true",
        help="create horizontal grid",
    )
    parser.add_argument(
        "--vertical", default=False, action="store_true", help="create vertical grid"
    )
    parser.add_argument(
        "--width", type=int, default=0, required=False, help="fixed grid width"
    )
    parser.add_argument(
        "--height", type=int, default=0, required=False, help="fixed grid height"
    )
    parser.add_argument(
        "--border", type=int, default=0, required=False, help="image border"
    )
    parser.add_argument(
        "--nolabels",
        default=False,
        action="store_true",
        help="do not print image labels",
    )
    parser.add_argument(
        "--debug",
        default=False,
    ...
    output = (
        params.output
        if params.output.lower().endswith(".jpg")
        else params.output + ".jpg"
    )
    if params.debug:
