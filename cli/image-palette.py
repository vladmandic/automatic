...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--color", type=int, default=20, help="color tolerance threshdold"
    )
    parser.add_argument(
        "--output", type=str, required=False, default="", help="folder to store images"
    )
    parser.add_argument(
        "--suffix",
        type=str,
        required=False,
        default="pallete",
        help="add suffix to image name",
    )
    parser.add_argument(
        "--grid",
        default=False,
        action="store_true",
        help="create grid of images before processing",
    )
    ...
    if not args.grid:
        for arg in args.input:
            if os.path.isfile(arg) and filetype.is_image(arg):
                ...
                fn = os.path.join(
                    args.output, pathlib.Path(arg).stem + "-" + args.suffix + ".jpg"
                )
                ...
                for root, _dirs, files in os.walk(arg):
                    for f in files:
                        if filetype.is_image(os.path.join(root, f)):
                            ...
                            fn = os.path.join(
                                args.output,
                                pathlib.Path(f).stem + "-" + args.suffix + ".jpg",
                            )
                            palette(image, args, fn)
    else:
        images = []
