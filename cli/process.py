...
class Result:
    def __init__(
        self, typ: str, fn: str, tag: str = None, requested: list | None = None
    ):
        self.type = typ
        self.input = fn
        self.output = ""
...
def detect_blur(image: Image):
    ...
    fftShift[
        cy - options.process.blur_samplesize : cy + options.process.blur_samplesize,
        cx - options.process.blur_samplesize : cx + options.process.blur_samplesize,
    ] = 0
...
def detect_simmilar(image: Image):
    img = image.resize(
        (options.process.similarity_size, options.process.similarity_size)
    )
    ...
    for i in all_images:
        val = ssim(
            data, i, data_range=255, channel_axis=None, gradient=False, full=False
        )
        if val > similarity:
            similarity = val
...
def segmentation(res: Result):
    ...
    if segmentation_model is None:
        segmentation_model = mp.solutions.selfie_segmentation.SelfieSegmentation(
            model_selection=options.process.segmentation_model
        )
    ...
    data = np.where(
        condition, data, background
    )  # consider using a joint bilateral filter instead of pure combine
...
def upscale_restore_image(res: Result, upscale: bool = False, restore: bool = False):
    ...
    if (
        res.image.width >= options.process.target_size
        and res.image.height >= options.process.target_size
    ):
        upscale = False
    if upscale:
...
def resize_image(res: Result):
    resized = res.image
    resized.thumbnail(
        (options.process.target_size, options.process.target_size), Image.HAMMING
    )
    res.image = resized
...
def square_image(res: Result):
    ...
    squared.paste(
        res.image, ((size - res.image.width) // 2, (size - res.image.height) // 2)
    )
    res.image = squared
...
def process_face(res: Result):
    ...
    if face_model is None:
        face_model = mp.solutions.face_detection.FaceDetection(
            min_detection_confidence=options.process.face_score,
            model_selection=options.process.face_model,
        )
    ...
    if (
        box.xmin < 0
        or box.ymin < 0
        or (box.width - box.xmin) > 1
        or (box.height - box.ymin) > 1
    ):
    ...
    h = min(
        res.image.height, (box.height + options.process.face_pad) * res.image.height
    )
    x = max(0, x)
...
def process_body(res: Result):
    ...
    if body_model is None:
        body_model = mp.solutions.pose.Pose(
            static_image_mode=True,
            min_detection_confidence=options.process.body_score,
            model_complexity=options.process.body_model,
        )
    ...
    if results.pose_landmarks is None:
        ...
        return res
    x0 = [
        res.image.width * (i.x - options.process.body_pad / 2)
        for i in results.pose_landmarks.landmark
        if i.visibility > options.process.body_visibility
    ]
    y0 = [
        res.image.height * (i.y - options.process.body_pad / 2)
        for i in results.pose_landmarks.landmark
        if i.visibility > options.process.body_visibility
    ]
    x1 = [
    ...
    if len(x0) < options.process.body_parts:
        ...
        res.image = None
        return res
    res.image = res.image.crop(
        (
            max(0, min(x0)),
            max(0, min(y0)),
            min(res.image.width, max(x1)),
            min(res.image.height, max(y1)),
        )
    )
    return res
...
def save_image(res: Result, folder: str):
    ...
    res.basename = (
        str(all_images_by_type[res.type]).rjust(3, "0")
        + "-"
        + res.type
        + "-"
        + res.basename
...
def file(filename: str, folder: str, tag=None, requested=[]):  # noqa: B006
    ...
    try:
        ...
        res.image = ImageOps.exif_transpose(
            res.image
    ...
    if (
        res.image.width < options.process.target_size
        or res.image.height < options.process.target_size
    ):
