...
async def txt2img():
    ...
    for i in range(len(data["images"])):
        data["images"][i] = Image.open(
            io.BytesIO(base64.b64decode(data["images"][i].split(",", 1)[0]))
        )
...
async def main():
        ...
        if ts > 0:
            ...
            if i == 0:
                ...
                log.info(
                    {
                        "batch": batch[i],
                        "its": round(options.steps / (ts / batch[i]), 2),
                        "img": round(ts / batch[i], 2),
                        "wall": round(ts, 2),
                        "peak": gb(peak),
                        "oom": oom > 0,
                    }
    ...
    if oom > 0:
        log.info(
            {
                "benchmark": "ended with oom so you should probably restart your automatic server now"
            }
        )
    await sdapi.close()
