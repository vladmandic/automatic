...
def mem_stats():
    ...
    mem = util.get_memory()
    peak = {
        "active": mem["gpu-active"]["peak"],
        "allocated": mem["gpu-allocated"]["peak"],
        "reserved": mem["gpu-reserved"]["peak"],
    }
...
def parse_args():
    ...
    group_main.add_argument(
        "--type",
        type=str,
        choices=["embedding", "ti", "lora", "lyco", "dreambooth", "hypernetwork"],
        default=None,
        required=True,
        help="training type",
    )
    group_main.add_argument(
        ...
        default="",
        required=False,
        help="base model to use for training, default: current loaded model",
    )
    group_main.add_argument(
        "--name", type=str, default=None, required=True, help="output filename"
    )
    group_main.add_argument(
        "--tag",
        type=str,
        default="person",
        required=False,
        help="primary tags, default: %(default)s",
    )
    ...
    group_data.add_argument(
        ...
        default=None,
        required=True,
        help="input folder with training images",
    )
    group_data.add_argument(
        "--interim",
        type=str,
        default="",
        required=False,
        help="where to store processed images, default is system temp/train",
    )
    group_data.add_argument(
        "--process",
        type=str,
        default="original,interrogate,resize,square",
        required=False,
        help=f"list of possible processing steps: {valid_steps}, default: %(default)s",
    )
    ...
    group_train.add_argument(
        ...
        default=1e-04,
        required=False,
        help="model learning rate, default: %(default)s",
    )
    group_train.add_argument(
        "--dim",
        type=int,
        default=32,
        required=False,
        help="network dimension or number of vectors, default: %(default)s",
    )
    ...
    group_train.add_argument(
        ...
        default=10,
        required=False,
        help="number of repeats per image, default: %(default)s",
    )
    group_train.add_argument(
        "--alpha",
        type=float,
        default=0,
        required=False,
        help="lora/lyco alpha for weights scaling, default: dim/2",
    )
    group_train.add_argument(
        "--algo",
        type=str,
        default=None,
        choices=["locon", "loha", "lokr", "ia3"],
        required=False,
        help="alternative lyco algoritm, default: %(default)s",
    )
    group_train.add_argument(
        "--args",
        type=str,
        default=None,
        required=False,
        help="lora/lyco additional network arguments, default: %(default)s",
    )
    ...
    group_other.add_argument(
        ...
        default=False,
        action="store_true",
        help="overwrite existing training, default: %(default)s",
    )
    group_other.add_argument(
        "--experimental",
        default=False,
        action="store_true",
        help="enable experimental options, default: %(default)s",
    )
    group_other.add_argument(
        "--debug",
        default=False,
...
async def training_loop():
    ...
    async def async_monitor():
        ...
        with tqdm(desc="train embedding", total=res.state.job_count) as pbar:
            while (
                res.state.job_no < res.state.job_count
                and not res.state.interrupted
                and not res.state.skipped
            ):
...
def train_lora():
    ...
    lora_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), os.pardir, "modules", "lora")
    )
    lycoris_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), os.pardir, "modules", "lycoris")
    )
...
def process_inputs():
    ...
    processing_options = (
        args.process.split(",") if isinstance(args.process, str) else args.process
    )
    ...
    steps = [
        step for step in processing_options if step in ["face", "body", "original"]
    ]
    process.reset()
    metadata = {}
    for step in steps:
        if step == "face":
            opts = [
                step for step in processing_options if step not in ["body", "original"]
            ]
        if step == "body":
            opts = [
                step
                for step in processing_options
                if step not in ["face", "original", "upscale", "restore"]
            ]  # body does not perform upscale or restore
        if step == "original":
            opts = [
                step
                for step in processing_options
            ...
            ]  # original does not perform most steps
        ...
        if args.type in ["lora", "lyco", "dreambooth"]:
            folder = os.path.join(
                args.process_dir, str(args.repeats) + "_" + concept
            )  # separate concepts per folder
        ...
        for f in files:
            ...
            if res.image:  # valid result
                ...
                rel_path = res.basename.replace(
                    os.path.commonpath([res.basename, args.process_dir]), ""
                )
                ...
                if options.lora.in_json is None:
                    with open(
                        res.output.replace(options.process.format, ".txt"),
                        "w",
                        encoding="utf-8",
                    ) as outfile:
                        outfile.write(res.caption)
            log.info(
                f"processing {'saved' if res.image is not None else 'skipped'}: {f} => {res.output} {res.ops} {res.message}"
            )
    folders = [
        os.path.join(args.process_dir, folder)
        for folder in os.listdir(args.process_dir)
        if os.path.isdir(os.path.join(args.process_dir, folder))
    ]
    log.info(f"input datasets {folders}")
    if options.lora.in_json is not None:
        with open(
            options.lora.in_json, "w", encoding="utf-8"
        ) as outfile:  # write json at the end only
            outfile.write(json.dumps(metadata, indent=2))
        for folder in folders:  # create latents
            import latents

            latents.create_vae_latents(
                util.Map({"input": folder, "json": options.lora.in_json})
            )
            latents.unload_vae()
    ...
    if args.gradient < 0:
        log.info(
            f"setting gradient accumulation to number of images: {results['total']}"
        )
...
def check_versions():
    ...
    if accelerate.__version__ != "0.19.0":
        log.error(
            f"invalid accelerate version: accelerate=0.19.0 found={accelerate.__version__}"
        )
        error = True
    ...
    if diffusers.__version__ != "0.10.2":
        log.error(
            f"invalid diffusers version: diffusers=0.10.2 found={diffusers.__version__}"
        )
        error = True
    if error:
