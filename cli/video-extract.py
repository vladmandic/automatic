...
def probe(src: str):
    ...
    video = Map(
        {
            "codec": res.get("codec_name", "unknown")
            + "/"
            + res.get("codec_tag_string", ""),
...
def extract(src: str, dst: str, rate: float = 0.015, fps: float = 0, start=0, end=0):
    ...
    for line in result.stderr.split("\n"):
        if "pts_time" in line:
            log.debug(
                {"extract": {"keyframe": line.strip().split(" ")[-1].split(":")[-1]}}
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0,
        required=False,
        help="extraction change rate threshold",
    )
    parser.add_argument(
        "--fps",
        type=float,
        default=0,
        required=False,
        help="extraction frames per second",
    )
    parser.add_argument(
        "--skipstart",
        type=float,
        default=1,
        required=False,
        help="skip time from start of video",
    )
    parser.add_argument(
        "--skipend",
        type=float,
        default=1,
        required=False,
        help="skip time to end of video",
    )
