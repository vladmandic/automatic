...
class LDSR:
    ...
    def super_resolution(self, image, steps=100, target_scale=2, half_attention=False):
        ...
        if down_sample_rate != 1:
            shared.log.info(
                f"LDSR Downsampling from [{width_og}, {height_og}] to [{width_downsampled_pre}, {height_downsampled_pre}]"
            )
            im_og = im_og.resize(
            ...
            shared.log.info(
                f"LDSR Downsample rate is 1 from {target_scale} / 4 (Not downsampling)"
            )
...
def make_convolutional_sample(
    ...
    if custom_shape is not None:
        ...
        shared.log.info(
            f"LDSR Generating {custom_shape[0]} samples of shape {custom_shape[1:]}"
        )
