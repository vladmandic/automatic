...
def run(
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"""{errdesc or 'Error running command'}: {command} code: {result.returncode}
...
def check_run(command):  # compatbility function
    result = subprocess.run(
        command, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
...
def run_pip(pkg, desc=None):  # compatbility function
    ...
    return run(
        f'"{sys.executable}" -m pip {pkg} --prefer-binary{index_url_line}',
...
if __name__ == "__main__":
    ...
    if installer.check_timestamp():
        ...
        if installer.errors == 0:
            ...
            installer.log.warning(
                f"See log file for more details: {installer.log_file}"
            )
    ...
    while True:
        try:
            ...
            requests = (
                instance.server_state.total_requests
                if hasattr(instance, "server_state")
                else 0
            )
        except Exception:
            alive = False
        ...
        if round(time.time()) % 120 == 0:
            installer.log.debug(
                f"Server alive={alive} Requests={requests} memory {get_memory_stats()} "
            )
        if not alive:
