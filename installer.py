                ...
                if len(p) > 1:
                    ...
                    if not exact:
                        if args.experimental:
                            log.warning(
                                f"Package allowing experimental: {p[0]} {version} required {p[1]}"
                            )
                        else:
                            log.warning(
...
def check_torch():
    ...
    log.debug(
        f"Torch overrides: cuda={args.use_cuda} rocm={args.use_rocm} ipex={args.use_ipex} diml={args.use_directml}"
    )
    log.debug(
        f"Torch allowed: cuda={allow_cuda} rocm={allow_rocm} ipex={allow_ipex} diml={allow_directml}"
    )
    ...
    if torch_command != "":
        ...
        or os.path.exists("/opt/rocm/bin/rocminfo")
        ...
        or shutil.which("sycl-ls.exe") is not None
        or os.environ.get("ONEAPI_ROOT") is not None
        or os.path.exists("/opt/intel/oneapi")
        or os.path.exists("C:/Program Files (x86)/Intel/oneAPI")
        or os.path.exists("C:/oneAPI")
    ):
        args.use_ipex = True  # pylint: disable=attribute-defined-outside-init
        log.info("Intel OneAPI Toolkit detected")
        if shutil.which("sycl-ls") is None and shutil.which("sycl-ls.exe") is None:
            log.error(
                "Intel OneAPI Toolkit is not activated! Activate OneAPI manually!"
            )
        ...
        if "linux" in sys.platform:
            torch_command = os.environ.get(
                "TORCH_COMMAND",
                "torch==2.0.1a0 torchvision==0.15.2a0 intel_extension_for_pytorch==2.0.110+xpu -f https://developer.intel.com/ipex-whl-stable-xpu",
            )
            os.environ.setdefault(
                "TENSORFLOW_PACKAGE",
                "tensorflow==2.13.0 intel-extension-for-tensorflow[gpu]",
            )
        else:
            torch_command = os.environ.get(
                "TORCH_COMMAND",
                "torch==2.0.0a0 torchvision==0.15.2a0 intel_extension_for_pytorch==2.0.110+gitba7f6c1 -f https://developer.intel.com/ipex-whl-stable-xpu",
            )
    else:
        machine = platform.machine()
        if sys.platform == "darwin":
            torch_command = os.environ.get(
                "TORCH_COMMAND", "torch==2.0.1 torchvision==0.15.2"
            )
        elif (
            allow_directml
            and args.use_directml
            and ("arm" not in machine and "aarch" not in machine)
        ):
                ...
                try:
                    if args.use_directml and allow_directml:
                        ...
                        for i in range(0, torch_directml.device_count()):
                            log.info(
                                f"Torch detected GPU: {torch_directml.device_name(i)}"
                            )
                except Exception:
...
def check_modified_files():
    ...
    try:
        ...
        files = [
            ...
            for x in files
            if len(x) > 0
            and (not x.startswith("extensions"))
            and (not x.startswith("wiki"))
...
def install_packages():
    ...
    invisiblewatermark_package = os.environ.get(
        "INVISIBLEWATERMARK_PACKAGE",
...
def install_repositories():
    ...
    clone(
        ...
        d("stable-diffusion-stability-ai"),
        stable_diffusion_commit,
    )
    ...
    clone(
        taming_transformers_repo, d("taming-transformers"), taming_transformers_commit
    )
...
def run_extension_installer(folder):
    ...
    try:
        ...
        result = subprocess.run(
            f'"{sys.executable}" "{path_installer}"',
            shell=True,
            env=env,
            check=False,
            stdout=subprocess.PIPE,
...
def list_extensions_folder(folder, quiet=False):
    ...
    enabled_extensions = [
        ...
        if x not in disabled_extensions and not x.startswith(".")
    ]
    if not quiet:
...
def install_extensions():
    ...
    pkgs = [
        f"{p.project_name}=={p._version}" for p in pkg_resources.working_set
...
def install_requirements():
    ...
    with open("requirements.txt", "r", encoding="utf8") as f:
        lines = [
            ...
            if line.strip() != "" and not line.startswith("#") and line is not None
        ]
        for line in lines:
            install(line)
    if args.profile:
    ...
    for folder in extension_folders:
        ...
        for ext in extensions:
            ...
            if not os.path.isdir(extension_dir):
                log.debug(
                    f"Extension listed as installed but folder missing: {extension_dir}"
                )
                continue
...
def check_version(offline=False, reset=True):  # pylint: disable=unused-argument
    ...
    try:
        commits = requests.get(
            "https://api.github.com/repos/vladmandic/automatic/branches/master",
            timeout=10,
        ).json()
...
def check_timestamp():
    ...
    if setup_time < extension_time:
        ok = False
    log.debug(
        f"Timestamps: version:{version_time} setup:{setup_time} extension:{extension_time}"
    )
    if args.reinstall:
        ok = False
    return ok
...
def add_args(parser):
    ...
    group.add_argument(
        ...
        default=False,
        action="store_true",
        help="Force re-check of requirements, default: %(default)s",
    )
    group.add_argument(
        "--quick",
        default=False,
        action="store_true",
        help="Run with startup sequence only, default: %(default)s",
    )
    group.add_argument(
        "--use-ipex",
        default=False,
    ...
    group.add_argument(
        ...
        default=False,
        action="store_true",
        help="Skips running Torch checks, default: %(default)s",
    )
    group.add_argument(
        "--experimental",
        default=False,
    ...
    group.add_argument(
        ...
        default=False,
        action="store_true",
        help="Force reinstallation of all requirements, default: %(default)s",
    )
    group.add_argument(
        "--test", default=False, action="store_true", help="Run test only and exit"
    )
    group.add_argument(
        "--version",
        default=False,
    ...
    group.add_argument(
        ...
        default=False,
        action="store_true",
        help="Ignore any errors and attempt to continue",
    )
    group.add_argument(
        "--safe",
        default=False,
        action="store_true",
        help="Run in safe mode with no user extensions",
    )
