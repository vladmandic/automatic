...
def ipex_init():
    ...
    torch.xpu.empty_cache = (
        torch.xpu.empty_cache
        if "WSL2" not in os.popen("uname -a").read()
        else lambda: None
    )
    ...
    CondFunc(
        "torch.load",
        lambda orig_func, f, map_location=None, *args, **kwargs: orig_func(
            f, shared.device, *args, **kwargs
        ),
        lambda orig_func, f, map_location=None, *args, **kwargs: (map_location is None)
        or (type(map_location) is torch.device and map_location.type == "cuda")
        or (type(map_location) is str and "cuda" in map_location),
    )
    ...
    CondFunc(
        "torch.bmm",
        lambda orig_func, input, mat2, *args, **kwargs: orig_func(
    ...
    CondFunc(
        ...
        lambda orig_func, input, normalized_shape=None, weight=None, *args, **kwargs: input.dtype
        != weight.data.dtype
    ...
    CondFunc(
        "torch.cat",
        lambda orig_func, input, *args, **kwargs: orig_func(
            [input[0].to(input[1].dtype), input[1], input[2].to(input[1].dtype)],
            *args,
            **kwargs
        ),
        lambda orig_func, input, *args, **kwargs: len(input) == 3
        and (input[0].dtype != input[1].dtype or input[2].dtype != input[1].dtype),
    )
    # Diffusers BF16:
    CondFunc(
        "torch.nn.functional.conv2d",
        lambda orig_func, input, weight, *args, **kwargs: orig_func(
            input.to(weight.data.dtype), weight, *args, **kwargs
        ),
        lambda orig_func, input, weight, *args, **kwargs: input.dtype
        != weight.data.dtype,
    )
    ...
    CondFunc(
        "torch.linalg.solve",
        lambda orig_func, A, B, *args, **kwargs: orig_func(
            A.to("cpu"), B.to("cpu"), *args, **kwargs
        ).to(shared.device),
        lambda orig_func, A, B, *args, **kwargs: A.device != torch.device("cpu")
        or B.device != torch.device("cpu"),
    )
    # SDE Samplers:
    CondFunc(
        "torch.Generator",
    ...
    CondFunc(
        ...
        lambda orig_func, input, weight, bias, *args, **kwargs: orig_func(
            ...
            else torch.ones(input.size()[1], device=shared.device),
            bias
            if bias is not None
            else torch.zeros(input.size()[1], device=shared.device),
            *args,
            **kwargs
        ),
