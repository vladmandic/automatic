...
def update_extension_list():
    ...
    try:
        with open(
            os.path.join(paths.script_path, "html", "extensions.json"),
            "r",
            encoding="utf-8",
        ) as f:
            extensions_list = json.loads(f.read())
            shared.log.debug(
                f'Extensions list loaded: {os.path.join(paths.script_path, "html", "extensions.json")}'
            )
    except Exception:
        shared.log.debug(
            f'Extensions list failed to load: {os.path.join(paths.script_path, "html", "extensions.json")}'
        )
    found = []
    ...
    not_matched = [
        extension for extension in extensions.extensions if extension not in found
    ]
...
def check_updates(_id_task, disable_list, search_text, sort_column):
    ...
    shared.log.info(
        f"Extensions update check: update={len(exts)} disabled={len(disable_list)}"
    )
    ...
    for ext in exts:
        ...
        try:
            ...
            if ext.can_update:
                ...
                shared.log.info(
                    f"Extensions updated: {ext.name} {ext.commit_hash[:8]} {datetime.utcfromtimestamp(commit_date)}"
                )
            else:
                commit_date = ext.commit_date or 1577836800
                shared.log.debug(
                    f"Extensions no update available: {ext.name} {ext.commit_hash[:8]} {datetime.utcfromtimestamp(commit_date)}"
                )
    ...
    return (
        ...
        "Extension update complete | Restart required",
    )
...
def install_extension_from_url(dirname, url, branch_name, search_text, sort_column):
    ...
    try:
        ...
        return [
            refresh_extensions_list_from_data(search_text, sort_column),
            html.escape(f"Extension installed: {target_dir} | Restart required"),
        ]
    finally:
        shutil.rmtree(tmpdir, True)
...
def install_extension(extension_to_install, search_text, sort_column):
    ...
    code, message = install_extension_from_url(
        None, extension_to_install, None, search_text, sort_column
    )
    return code, message
...
def uninstall_extension(extension_path, search_text, sort_column):
    ...
    found = [
        extension
        for extension in extensions.extensions
        if os.path.abspath(extension.path) == os.path.abspath(extension_path)
    ]
    if len(found) > 0 and os.path.isdir(extension_path):
        ...
        shared.log.warning(
            f"Extension uninstall cannot find extension: {extension_path}"
        )
        code = refresh_extensions_list_from_data(search_text, sort_column)
...
def update_extension(extension_path, search_text, sort_column):
    ...
    for ext in exts:
        shared.log.debug(
            f"Extensions update start: {ext.name} {ext.commit_hash} {ext.commit_date}"
        )
        shared.state.textinfo = ext.name
        try:
            ext.check_updates()
            ...
            errors.display(e, f"extensions check update: {ext.name}")
        shared.log.debug(
            f"Extensions update finish: {ext.name} {ext.commit_hash} {ext.commit_date}"
        )
        shared.state.nextjob()
    return (
        refresh_extensions_list_from_data(search_text, sort_column),
        f"Extension updated | {extension_path} | Restart required",
    )
...
def refresh_extensions_list(search_text, sort_column):
    ...
    try:
        ...
        with open(
            ...
            shared.log.debug(
                f"Updated extensions list: {len(extensions_list)} {extensions_index}"
            )
    except Exception as e:
    ...
    return (
        code,
        f"Extensions | {len(extensions.extensions)} registered | {len(extensions_list)} available",
    )
...
def refresh_extensions_list_from_data(search_text, sort_column):
    ...
    for ext in extensions_list:
        ...
        ext["commit_date"] = (
            extension[0].commit_date if len(extension) > 0 else 1577836800
        )
        ...
        ext[
            "sort_default"
        ] = f"{'1' if ext['is_builtin'] else '0'}{'1' if ext['installed'] else '0'}{ext.get('updated', '2000-01-01T00:00')}"
    ...
    for ext in sorted(extensions_list, key=sort_function, reverse=sort_reverse):
        ...
        update_available = (
            ...
            & (installed)
            & (
                datetime.utcfromtimestamp(commit_date + 60 * 60)
                < datetime.fromisoformat(
                    ext.get("updated", "2000-01-01T00:00:00.000Z")[:-1]
                )
            )
        )
        ext[
            "sort_user"
        ] = f"{'0' if ext['is_builtin'] else '1'}{'1' if ext['installed'] else '0'}{ext.get('name', '')}"
        ext[
            "sort_enabled"
        ] = f"{'0' if ext['enabled'] else '1'}{'1' if ext['is_builtin'] else '0'}{'1' if ext['installed'] else '0'}{ext.get('updated', '2000-01-01T00:00')}"
        ext[
            "sort_update"
        ] = f"{'1' if update_available else '0'}{'1' if ext['installed'] else '0'}{ext.get('updated', '2000-01-01T00:00')}"
        timedelta = datetime.now() - datetime.fromisoformat(
            ext.get("created", "2000-01-01T00:00Z")[:-1]
        )
        ...
        if search_text and search_text.strip():
            if (
                search_text.lower() not in html.escape(name).lower()
                and search_text.lower() not in html.escape(description).lower()
                and search_text.lower() not in html.escape(tags_string).lower()
            ):
                stats["hidden"] += 1
                visible = "none"
        stats["processed"] += 1
    ...
    code += "</tbody></table>"
    shared.log.debug(
        f'Extension list refresh: processed={stats["processed"]} installed={stats["installed"]} enabled={stats["enabled"]} disabled={stats["installed"] - stats["enabled"]} visible={stats["processed"] - stats["hidden"]} hidden={stats["hidden"]}'
    )
    return code
        ...
        with gr.Tabs(elem_id="tabs_extensions"):
            with gr.TabItem("Manage extensions", id="manage"):
                with gr.Row(elem_id="extensions_installed_top"):
                    ...
                    uninstall_extension_button = gr.Button(
                        elem_id="uninstall_extension_button", visible=False
                    )
                    update_extension_button = gr.Button(
                        elem_id="update_extension_button", visible=False
                    )
                    with gr.Column(scale=4):
                        search_text = gr.Text(label="Search")
                        info = gr.HTML(
                            "Note: After any operation such as install/uninstall or enable/disable, please restart the server"
                        )
                    with gr.Column(scale=1):
                        sort_column = gr.Dropdown(
                            value="default",
                            label="Sort by",
                            choices=list(sort_ordering.keys()),
                            multiselect=False,
                        )
                    with gr.Column(scale=1):
                        refresh_extensions_button = gr.Button(
                            value="Refresh extension list", variant="primary"
                        )
                        check = gr.Button(
                            value="Update installed extensions", variant="primary"
                        )
                        apply = gr.Button(
                            value="Apply changes & restart server", variant="primary"
                        )
                update_extension_list()
                extensions_table = gr.HTML(
                    refresh_extensions_list_from_data(
                        search_text.value, sort_column.value
                    )
                )
                check.click(
                ...
                search_text.change(
                    fn=modules.ui.wrap_gradio_call(
                        search_extensions, extra_outputs=[gr.update(), gr.update()]
                    ),
                    inputs=[search_text, sort_column],
                    outputs=[extensions_table, info],
                )
            ...
            with gr.TabItem("Manual install", id="install_from_url"):
                ...
                install_button.click(
                    fn=modules.ui.wrap_gradio_call(
                        install_extension_from_url, extra_outputs=[gr.update()]
                    ),
                    inputs=[
                        install_dirname,
                        install_url,
