...
def save_image(image, fn, ext):
    ...
    if image_format == "PNG":
        ...
        image.save(
            fn, format=image_format, quality=shared.opts.jpeg_quality, exif=exif_bytes
        )
...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        if shared.cmd_opts.auth:
            for auth in shared.cmd_opts.auth.split(","):
                ...
                self.credentials[user.replace('"', "").strip()] = password.replace(
                    '"', ""
                ).strip()
        ...
        self.add_api_route(
            "/sdapi/v1/refresh-vaes", self.refresh_vaes, methods=["POST"]
        )
        self.add_api_route(
        ...
        self.add_api_route(
            "/sdapi/v1/log", self.get_log_buffer, methods=["GET"], response_model=List
        )  # bypass auth
    ...
    def get_log_buffer(self, req: models.LogRequest = Depends()):
        lines = (
            shared.log.buffer[: req.lines]
            if req.lines > 0
            else shared.log.buffer.copy()
        )
        if req.clear:
        ...
        if request.alwayson_scripts and (len(request.alwayson_scripts) > 0):
            for alwayson_script_name in request.alwayson_scripts.keys():
                ...
                if alwayson_script is None:
                    raise HTTPException(
                        status_code=422,
                        detail=f"Always on script not found: {alwayson_script_name}",
                    )
                if not alwayson_script.alwayson:
                    raise HTTPException(
                        status_code=422,
                        detail=f"Selectable script cannot be in always on params: {alwayson_script_name}",
                    )
    ...
    def text2imgapi(self, txt2imgreq: models.StableDiffusionTxt2ImgProcessingAPI):
        ...
        with self.queue_lock:
            ...
            p.outpath_grids = (
                shared.opts.outdir_grids or shared.opts.outdir_txt2img_grids
            )
            p.outpath_samples = (
                shared.opts.outdir_samples or shared.opts.outdir_txt2img_samples
            )
            shared.state.begin()
    ...
    def get_config(self):
        ...
        for k in shared.opts.data.keys():
            if shared.opts.data_labels.get(k) is not None:
                options.update(
                    {k: shared.opts.data.get(k, shared.opts.data_labels.get(k).default)}
                )
            else:
    ...
    def create_hypernetwork(self, args: dict):
        try:
            shared.state.begin()
            filename = create_hypernetwork(
                **args
            )  # create empty embedding # pylint: disable=E1111
            shared.state.end()
    ...
    def train_embedding(self, args: dict):
        try:
            ...
            try:
                _embedding, filename = train_embedding(
                    **args
