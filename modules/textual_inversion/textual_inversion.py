...
class EmbeddingDatabase:
    ...
    def load_diffusers_embedding(self, filename: str, path: str):
        ...
        try:
            pipe.load_textual_inversion(
                path, cache_dir=shared.opts.data["diffusers_dir"], local_files_only=True
            )
        ...
        try:
            ...
            text_inv_tokens = [
                t
                for t in text_inv_tokens
                if not (len(t.split("_")) > 1 and t.split("_")[-1].isdigit())
            ]
        except Exception:
            text_inv_tokens = []
    ...
    def load_from_file(self, path, filename):
        ...
        # diffuser concepts
            ...
            if len(data.keys()) != 1:
                shared.log.warning(
                    f"Embedding file has multiple terms in it: {filename}"
                )
    ...
    def load_textual_inversion_embeddings(self, force_reload=False):
        ...
        if self.previously_displayed_embeddings != displayed_embeddings:
            ...
            shared.log.info(
                f"Embeddings: loaded={len(self.word_embeddings)} skipped={len(self.skipped_embeddings)}"
            )
...
def create_embedding(name, num_vectors_per_token, overwrite_old, init_text="*"):
    ...
    if not overwrite_old and os.path.exists(fn):
        ...
        embedding.save(fn)
        shared.log.info(
            f"Created embedding: {fn} vectors {num_vectors_per_token} init {init_text}"
        )
    return fn
...
def write_loss(log_directory, filename, step, epoch_len, values):
    ...
    with open(
        os.path.join(log_directory, filename), "a+", newline="", encoding="utf-8"
    ) as fout:
...
def train_embedding(
    ...
    if log_directory is None or log_directory == "":
        log_directory = (
            f"{os.path.join(shared.cmd_opts.data_dir, 'train/log/embeddings')}"
        )
            ...
            for j, batch in enumerate(dl):
                ...
                if images_dir is not None and steps_done % create_image_every == 0:
                    ...
                    if image is not None:
                        ...
                        last_saved_image, _last_text_info = images.save_image(
                            image,
                            images_dir,
                            "",
                            p.seed,
