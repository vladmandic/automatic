...
def process_diffusers(p: StableDiffusionProcessing, seeds, prompts, negative_prompts):
    ...
    if sd_models.get_diffusers_task(shared.sd_model) == sd_models.DiffusersTaskType.TEXT_2_IMAGE:
        ...
        # for sd we can be sure that VAE encoded images exist, other models might not have a VAE
        ...
        # for sd we can be sure that VAE encoded images exist, other models might not have a VAE
        image = p.init_latent if "StableDiffusion" in shared.sd_model.__class__.__name__ else p.init_images
        # if we use latents we don't need a mask because it's already part of the latents
        mask = torch.ones_like(image) if "StableDiffusion" in shared.sd_model.__class__.__name__ else p.mask

        task_specific_kwargs = {"image": image, "mask_image": mask, "strength": p.denoising_strength, "height": p.height, "width": p.width}
