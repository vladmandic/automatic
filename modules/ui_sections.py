    ...
    with gr.Accordion(open=False, label="Refine", elem_id=f"{tab}_second_pass", elem_classes=["small-accordion"]):
        with gr.Group():
            ...
            with gr.Row(elem_id=f"{tab}_hires_fix_row2", variant="compact"):
                ...
                hr_sampler_index = gr.Dropdown(label='Secondary sampler', elem_id=f"{tab}_sampling_alt", choices=[x.name for x in sd_samplers.samplers], value="Same as primary" if shared.Backend.DIFFUSERS else 'Default', type="index")
