...
class UiLoadsave:
    ...
    def iter_changes(self, values):
        ...
        for i, name in enumerate(self.component_mapping):
            ...
            if (new_value == default_value) and (old_value is None):
                continue
            log.debug(
                f"Settings: name={name} component={component} old={old_value} default={default_value} new={new_value}"
            )
    ...
    def ui_view(self, *values):
        text = [
            '<table style="width: -webkit-fill-available"><thead style="font-size: 110%; border-style: solid; border-bottom: 1px var(--button-primary-border-color) solid"><tr><th>Variable</th><th>User value</th><th>New value</th><th>Default value</th></thead><tbody>'
        ]
        for path, old_value, new_value, default_value in self.iter_changes(values):
            ...
            text.append(
                f"<tr><td>{path}</td><td>{old_value}</td><td>{new_value}</td><td>{default_value}</td></tr>"
            )
        if len(text) == 1:
    ...
    def create_ui(self):
        ...
        gr.HTML(
            f"Review changed values and apply them as new user interface defaults<br>Config file: {self.filename}"
        )
        with gr.Row():
            ...
            self.ui_defaults_apply = gr.Button(
                value="Set new defaults", elem_id="ui_defaults_apply", variant="primary"
            )
            self.ui_defaults_restore = gr.Button(
                value="Restore system defaults",
                elem_id="ui_defaults_restore",
                variant="primary",
            )
    ...
    def setup_ui(self):
        ...
        self.ui_defaults_restore.click(
            fn=self.ui_restore, inputs=[], outputs=[self.ui_defaults_review]
        )
