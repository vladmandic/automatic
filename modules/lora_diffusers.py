...
def unload_diffusers_lora():
    try:
        ...
        if shared.opts.diffusers_lora_loader == "diffusers default":
            ...
            proc_cls_name = next(
                iter(pipe.unet.attn_processors.values())
            ).__class__.__name__
            non_lora_proc_cls = getattr(
                diffusers.models.attention_processor, proc_cls_name
            )  # [len("LORA"):])
    ...
    try:
        ...
        if shared.opts.diffusers_lora_loader == "diffusers default":
            ...
            lora_network: LoRANetwork = create_network_from_weights(
                text_encoders, pipe.unet, lora_sd, multiplier=strength
            )
