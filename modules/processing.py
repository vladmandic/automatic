...
class StableDiffusionProcessing:
    ...
    def inpainting_image_conditioning(
        ...
        image_conditioning = image_conditioning.to(
            device=shared.device, dtype=source_image.dtype
        )
        return image_conditioning

    def diffusers_image_conditioning(
        self, _source_image, latent_image, _image_mask=None
    ):
    ...
    def img2img_image_conditioning(self, source_image, latent_image, image_mask=None):
        ...
        if shared.backend == shared.Backend.DIFFUSERS:
            return self.diffusers_image_conditioning(
                source_image, latent_image, image_mask
            )
        if isinstance(self.sd_model, LatentDepth2ImageDiffusion):
            return self.depth2img_image_conditioning(source_image)
        if (
            hasattr(self.sd_model, "cond_stage_key")
            and self.sd_model.cond_stage_key == "edit"
        ):
            return self.edit_image_conditioning(source_image)
        if hasattr(
            self.sampler, "conditioning_key"
        ) and self.sampler.conditioning_key in {"hybrid", "concat"}:
            return self.inpainting_image_conditioning(
                source_image, latent_image, image_mask=image_mask
            )
        if (
            hasattr(self.sampler, "conditioning_key")
            and self.sampler.conditioning_key == "crossattn-adm"
        ):
    ...
    def get_token_merging_ratio(self, for_hr=False):
        if for_hr:
            return (
                ...
                or shared.opts.token_merging_ratio_hr
                or self.token_merging_ratio
                or shared.opts.token_merging_ratio
            )
...
class Processed:
    def __init__(
        ...
        self.face_restoration_model = (
            shared.opts.face_restoration_model if p.restore_faces else None
        )
...
def create_random_tensors(
    ...
    if (
        ...
        and (
            len(seeds) > 1
            and shared.opts.enable_batch_seeds
            or eta_noise_seed_delta > 0
        )
    ):
...
def create_infotext(
    ...
    generation_params = {
        ...
        "Model": None
        if not shared.opts.add_model_name_to_info
        ...
        "Model hash": getattr(
            ...
            "sd_model_hash",
            None
            if not shared.opts.add_model_hash_to_info
            or not shared.sd_model.sd_model_hash
            else shared.sd_model.sd_model_hash,
        ),
        "Refiner": None
        if not shared.opts.add_model_name_to_info
        or not shared.sd_refiner
        or not shared.sd_refiner.sd_checkpoint_info.model_name
        else shared.sd_refiner.sd_checkpoint_info.model_name.replace(",", "").replace(
            ":", ""
        ),
        "VAE": None
        if not shared.opts.add_model_name_to_info or sd_vae.loaded_vae_file is None
        ...
        # ensd
        "ENSD": shared.opts.eta_noise_seed_delta
        if shared.opts.eta_noise_seed_delta != 0
        and sd_samplers_common.is_sampler_using_eta_noise_seed_delta(p)
        else None,
        # enable_hr
        ...
        # restore_faces
        "Face restoration": shared.opts.face_restoration_model
        if p.restore_faces
        else None,
        # sdnext
        "Version": git_commit,
    ...
    token_merging_ratio_hr = (
        p.get_token_merging_ratio(for_hr=True) if p.enable_hr else None
    )
    generation_params["Token merging ratio"] = (
        token_merging_ratio if token_merging_ratio != 0 else None
    )
    generation_params["Token merging ratio hr"] = (
        token_merging_ratio_hr if token_merging_ratio_hr != 0 else None
    )
    ...
    negative_prompt_text = (
        f"\nNegative prompt: {all_negative_prompts[index]}"
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    try:
        ...
        if (
            p.override_settings.get("sd_model_checkpoint", None) is not None
            and sd_models.checkpoint_aliases.get(
    ...
    with torch.no_grad(), ema_scope_context():
        with devices.autocast():
            ...
            if (
                ...
                and shared.opts.show_progress_type == "Approximate NN"
        ...
        for n in range(p.n_iter):
            ...
            if shared.backend == shared.Backend.ORIGINAL:
                ...
                try:
                    ...
                    if (
                        not shared.opts.no_half
                        and not shared.opts.no_half_vae
                        and shared.cmd_opts.rollback_vae
                    ):
                        shared.log.warning("Tensor with all NaNs was produced in VAE")
                        devices.dtype_vae = torch.bfloat16
                        vae_file, vae_source = sd_vae.resolve_vae(
                            p.sd_model.sd_model_checkpoint
                        )
                        sd_vae.load_vae(p.sd_model, vae_file, vae_source)
                        x_samples_ddim = [
                            decode_first_stage(
                                p.sd_model,
                                samples_ddim[i : i + 1].to(dtype=devices.dtype_vae),
                            )[0].cpu()
                            for i in range(samples_ddim.size(0))
                        ]
                        for x in x_samples_ddim:
                ...
                x_samples_ddim = process_diffusers(
                    p, p.seeds, p.prompts, p.negative_prompts
                )
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if p.restore_faces:
                    if (
                        ...
                        and shared.opts.save_images_before_face_restoration
                    ):
                ...
                if p.color_corrections is not None and i < len(p.color_corrections):
                    if (
                        shared.opts.save
                        and not p.do_not_save_samples
                        and shared.opts.save_images_before_color_correction
                    ):
                ...
                if (
                    ...
                    and any(
                        [
                            shared.opts.save_mask,
                            shared.opts.save_mask_composite,
                            shared.opts.return_mask,
                            shared.opts.return_mask_composite,
                        ]
                    )
                ):
                    ...
                    image_mask_composite = Image.composite(
                        ...
                        images.resize_image(
                            3, p.mask_for_overlay, image.width, image.height
                        ).convert("L"),
                    ).convert("RGBA")
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        if shared.backend == shared.Backend.DIFFUSERS:
            sd_models.set_diffuser_pipe(
                self.sd_model, sd_models.DiffusersTaskType.TEXT_2_IMAGE
            )
    ...
    def init_hr(self):
        if (
            shared.opts.use_old_hires_fix_width_height
    ...
    def sample(
        ...
        def save_intermediate(image, index):
            ...
            if (
                ...
                or not shared.opts.save_images_before_highres_fix
            ):
                return
        ...
        if latent_scale_mode is not None:
            ...
            if (
                ...
                image_conditioning = self.img2img_image_conditioning(
                    decode_first_stage(
                        self.sd_model, samples.to(dtype=devices.dtype_vae)
                    ),
                    samples,
                )
            else:
                image_conditioning = self.txt2img_image_conditioning(
                    samples.to(dtype=devices.dtype_vae)
                )
        else:
            decoded_samples = decode_first_stage(
                self.sd_model, samples.to(dtype=devices.dtype_vae)
            )
            ...
            decoded_samples = decoded_samples.to(
                device=shared.device, dtype=devices.dtype_vae
            )
                ...
                samples = torch.stack(
                    [
                        self.sd_model.get_first_stage_encoding(
                            self.sd_model.encode_first_stage(
                                torch.unsqueeze(decoded_sample, 0)
                            )
                        )[0]
        ...
        self.sampler = sd_samplers.create_sampler(
            self.latent_sampler or self.sampler_name, self.sd_model
        )
        samples = samples[
            :,
            :,
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        if shared.backend == shared.Backend.DIFFUSERS and self.image_mask is None:
            sd_models.set_diffuser_pipe(
                self.sd_model, sd_models.DiffusersTaskType.IMAGE_2_IMAGE
            )
        elif shared.backend == shared.Backend.DIFFUSERS and self.image_mask is not None:
            sd_models.set_diffuser_pipe(
                self.sd_model, sd_models.DiffusersTaskType.INPAINTING
            )
        ...
        for img in self.init_images:
            ...
            if shared.opts.save_init_img:
                self.init_img_hash = hashlib.md5(
                    img.tobytes()
                ).hexdigest()  # pylint: disable=attribute-defined-outside-init
                images.save_image(
                    img,
            ...
            if image_mask is not None:
                ...
                image_masked.paste(
                    ...
                    if self.mask_for_overlay is not None
                    else None,
                )
        ...
        if image_mask is not None:
            ...
            self.mask = torch.asarray(1.0 - latmask).to(
                device=shared.device, dtype=self.sd_model.dtype
            )
    ...
    def get_token_merging_ratio(self, for_hr=False):
        return (
            ...
            or (
                ...
                and shared.opts.token_merging_ratio
            )
            or shared.opts.token_merging_ratio_img2img
            or shared.opts.token_merging_ratio
        )
