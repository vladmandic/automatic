...
def load_vae(model, vae_file=None, vae_source="from unknown source"):
    ...
    if vae_file:
        if cache_enabled and vae_file in checkpoints_loaded:
            ...
            shared.log.info(
                f"Loading VAE weights: {vae_source}: cached {get_filename(vae_file)}"
            )
    ...
    try:
        ...
        if os.path.isfile(vae_file):
            if shared.opts.diffusers_pipeline == "Stable Diffusion XL":
                ...
                shared.log.warning(
                    "Using SDXL VAE loaded from singular file will result in low contrast images."
                )
            vae = diffusers.AutoencoderKL.from_single_file(vae_file)
            vae = vae.to(devices.dtype_vae)
        else:
            vae = diffusers.AutoencoderKL.from_pretrained(
                vae_file, **diffusers_load_config
            )
