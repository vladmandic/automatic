...
def setup_middleware(app: FastAPI, cmd_opts):
    ...
    if cmd_opts.cors_origins and cmd_opts.cors_regex:
        app.add_middleware(
            CORSMiddleware,
            allow_origins=cmd_opts.cors_origins.split(","),
            allow_origin_regex=cmd_opts.cors_regex,
            allow_methods=["*"],
    ...
    async def log_and_time(req: Request, call_next):
        try:
            ...
            if (cmd_opts.api_log or cmd_opts.api_only) and endpoint.startswith(
                "/sdapi"
            ):
                log.info(
                    "API {t} {code} {prot}/{ver} {method} {endpoint} {cli} {duration}".format(  # pylint: disable=consider-using-f-string, logging-format-interpolation
    ...
    def handle_exception(req: Request, e: Exception):
        ...
        if (
            not isinstance(e, HTTPException) and err["error"] != "TypeError"
