...
from modules.paths_internal import models_path, script_path, data_path, sd_configs_path, sd_default_config, \
    sd_model_file, default_sd_model_file, extensions_dir, extensions_builtin_dir  # pylint: disable=W0611
...
cmd_opts.disable_extension_access = (
                                                cmd_opts.share or cmd_opts.listen or cmd_opts.server_name) and not cmd_opts.enable_insecure
...
options_templates.update(options_section(('sd', "Stable Diffusion"), {
    "sd_model_checkpoint": OptionInfo(default_checkpoint, "Stable Diffusion checkpoint", gr.Dropdown,
    ...
    "enable_quantization": OptionInfo(True,
    ...
    "cross_attention_optimization": OptionInfo("Scaled-Dot-Product", "Cross-attention optimization method", gr.Radio,
                                               lambda: {"choices": shared_items.list_crossattention()}),
    "cross_attention_options": OptionInfo([], "Cross-attention advanced options", gr.CheckboxGroup, lambda: {
        "choices": ['xFormers enable flash Attention', 'SDP disable memory attention']}),
    "sub_quad_q_chunk_size": OptionInfo(512,
                                        "Sub-quadratic cross-attention query chunk size for the  layer optimization to use",
                                        gr.Slider, {"minimum": 16, "maximum": 8192, "step": 8}),
    "sub_quad_kv_chunk_size": OptionInfo(512,
                                         "Sub-quadratic cross-attentionkv chunk size for the sub-quadratic cross-attention layer optimization to use",
                                         gr.Slider, {"minimum": 0, "maximum": 8192, "step": 8}),
    "sub_quad_chunk_threshold": OptionInfo(80, "Sub-quadratic cross-attention percentage of VRAM chunking threshold",
                                           gr.Slider, {"minimum": 0, "maximum": 100, "step": 1}),
    "always_batch_cond_uncond": OptionInfo(False,
...
options_templates.update(options_section(('system-paths', "System Paths"), {
    ...
    "ckpt_dir": OptionInfo(os.path.join(paths.models_path, 'Stable-diffusion'),
    ...
    "embeddings_dir": OptionInfo(os.path.join(paths.models_path, 'embeddings'),
                                 "Embeddings directory for textual inversion"),
    "embeddings_templates_dir": OptionInfo(os.path.join(paths.script_path, 'train/templates'),
    ...
    "codeformer_models_path": OptionInfo(os.path.join(paths.models_path, 'Codeformer'),
                                         "Path to directory with codeformer model file(s)."),
    "gfpgan_models_path": OptionInfo(os.path.join(paths.models_path, 'GFPGAN'),
                                     "Path to directory with GFPGAN model file(s)"),
    "esrgan_models_path": OptionInfo(os.path.join(paths.models_path, 'ESRGAN'),
                                     "Path to directory with ESRGAN model file(s)"),
    "bsrgan_models_path": OptionInfo(os.path.join(paths.models_path, 'BSRGAN'),
    ...
    "scunet_models_path": OptionInfo(os.path.join(paths.models_path, 'ScuNET'),
                                     "Path to directory with ScuNET model file(s)"),
    "swinir_models_path": OptionInfo(os.path.join(paths.models_path, 'SwinIR'),
                                     "Path to directory with SwinIR model file(s)"),
    "ldsr_models_path": OptionInfo(os.path.join(paths.models_path, 'LDSR'),
                                   "Path to directory with LDSR model file(s)"),
    "clip_models_path": OptionInfo(os.path.join(paths.models_path, 'CLIP'),
...
options_templates.update(options_section(('saving-images', "Image options"), {
    ...
    "save_images_before_color_correction": OptionInfo(True,
                                                      "Save a copy of image before applying color correction to img2img results"),
    "save_mask": OptionInfo(False, "For inpainting, save a copy of the greyscale mask"),
    "save_mask_composite": OptionInfo(False, "For inpainting, save a masked composite"),
    "jpeg_quality": OptionInfo(85, "Quality for saved jpeg images", gr.Slider,
...
options_templates.update(options_section(('saving-paths', "Image Paths"), {
    ...
    "outdir_txt2img_samples": OptionInfo("outputs/text", 'Output directory for txt2img images',
                                         component_args=hide_dirs),
    "outdir_img2img_samples": OptionInfo("outputs/image", 'Output directory for img2img images',
                                         component_args=hide_dirs),
    "outdir_extras_samples": OptionInfo("outputs/extras", 'Output directory for images from extras tab',
                                        component_args=hide_dirs),
    ...
    "outdir_save": OptionInfo("outputs/save", "Directory for saving images using the Save button",
                              component_args=hide_dirs),
}))
...
if sys.platform == "darwin":
    # Provide out-of-the-box support for macOS.
...
options_templates.update(options_section(('cuda', "CUDA Settings"), {
    ...
    "rollback_vae": OptionInfo(False,
                               "Attempt to roll back VAE when produced NaN values, requires NaN check (experimental)"),
    ...
    "cuda_compile_mode": OptionInfo("none", "Model compile mode (experimental)", gr.Radio,
                                    lambda: {"choices": ['none', 'inductor', 'cudagraphs', 'aot_ts_nvfuser']}),
}))
...
options_templates.update(options_section(('upscaling', "Upscaling"), {
    ...
    "upscaler_for_img2img": OptionInfo("SwinIR_4x", "Upscaler for img2img", gr.Dropdown,
...
options_templates.update(options_section(('face-restoration', "Face restoration"), {
    ...
    "code_former_weight": OptionInfo(0.2, "CodeFormer weight parameter; 0 = maximum effect; 1 = minimum effect",
                                     gr.Slider, {"minimum": 0, "maximum": 1, "step": 0.01}),
    "face_restoration_unload": OptionInfo(False, "Move face restoration model from VRAM into RAM after processing"),
}))

options_templates.update(options_section(('system', "System"), {
    "memmon_poll_rate": OptionInfo(2, "VRAM usage polls per second during generation. Set to 0 to disable.", gr.Slider,
                                   {"minimum": 0, "maximum": 40, "step": 1}),
    "multiple_tqdm": OptionInfo(False,
...
options_templates.update(options_section(('training', "Training"), {
    ...
    "training_write_csv_every": OptionInfo(0,
...
options_templates.update(options_section(('interrogate', "Interrogate Options"), {
    ...
    "interrogate_return_ranks": OptionInfo(True,
    ...
    "interrogate_clip_num_beams": OptionInfo(1, "Interrogate: num_beams for BLIP", gr.Slider,
                                             {"minimum": 1, "maximum": 16, "step": 1}),
    "interrogate_clip_min_length": OptionInfo(32, "Interrogate: minimum description length (excluding artists, etc..)",
                                              gr.Slider, {"minimum": 1, "maximum": 128, "step": 1}),
    "interrogate_clip_max_length": OptionInfo(192, "Interrogate: maximum description length", gr.Slider,
    ...
    "interrogate_clip_skip_categories": OptionInfo(["artists", "movements", "flavors"], "CLIP: skip inquire categories",
                                                   gr.CheckboxGroup,
                                                   lambda: {"choices": modules.interrogate.category_types()},
                                                   refresh=modules.interrogate.category_types),
    "interrogate_deepbooru_score_threshold": OptionInfo(0.65, "Interrogate: deepbooru score threshold", gr.Slider,
...
options_templates.update(options_section(('ui', "User interface"), {
    "gradio_theme": OptionInfo("black-orange", "UI theme", gr.Dropdown, lambda: {"choices": list_themes()},
...
options_templates.update(options_section(('ui', "Live previews"), {
    ...
    "show_progress_every_n_steps": OptionInfo(1,
    ...
    "live_preview_content": OptionInfo("Combined", "Live preview subject", gr.Radio,
...
options_templates.update(options_section(('sampler-params', "Sampler parameters"), {
    "show_samplers": OptionInfo(
        ["Euler a", "UniPC", "DDIM", "DPM++ SDE", "DPM++ SDE", "DPM2 Karras", "DPM++ 2M Karras"],
        "Show samplers in user interface", gr.CheckboxGroup, lambda: {"choices": [x.name for x in list_samplers()]}),
    "eta_ancestral": OptionInfo(1.0, "Noise multiplier for ancestral samplers (eta)", gr.Slider,
                                {"minimum": 0.0, "maximum": 1.0, "step": 0.01}),
    "eta_ddim": OptionInfo(0.0, "Noise multiplier for DDIM (eta)", gr.Slider,
                           {"minimum": 0.0, "maximum": 1.0, "step": 0.01}),
...
options_templates.update(options_section(('token_merging', 'Token Merging'), {
    "token_merging": OptionInfo(False,
                                "Enable redundant token merging via tomesd. This can provide significant speed and memory improvements.",
                                gr.Checkbox),
    "token_merging_ratio": OptionInfo(0.5, "Merging Ratio", gr.Slider, {"minimum": 0, "maximum": 0.9, "step": 0.1}),
    "token_merging_hr_only": OptionInfo(True,
                                        "Apply only to high-res fix pass. Disabling can yield a ~20-35% speedup on contemporary resolutions.",
                                        gr.Checkbox),
    "token_merging_ratio_hr": OptionInfo(0.5,
                                         "Merging Ratio (high-res pass) - If 'Apply only to high-res' is enabled, this will always be the ratio used.",
                                         gr.Slider, {"minimum": 0, "maximum": 0.9, "step": 0.1}),
    "token_merging_random": OptionInfo(False,
                                       "Use random perturbations - Can improve outputs for certain samplers. For others, it may cause visual artifacting.",
                                       gr.Checkbox),
    ...
    "token_merging_maximum_down_sampling": OptionInfo(1, "Maximum down sampling", gr.Dropdown,
                                                      lambda: {"choices": ["1", "2", "4", "8"]}),
