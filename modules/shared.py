...
pipelines = [
    ...
    "Kandinsky V1 Img2Img",
    "Kandinsky V2 Img2Img",
    ...
    def do_set_current_image(self):
        ...
        try:
            image = (
                ...
                else modules.sd_samplers.sample_to_image(self.current_latent)
            )
...
backend = (
    Backend.DIFFUSERS
    if (cmd_opts.backend is not None) and (cmd_opts.backend.lower() == "diffusers")
    else Backend.ORIGINAL
)  # initial since we don't have opts loaded yet
...
class OptionInfo:
    def __init__(
        ...
        self.comment_before = (
            comment_before  # HTML text that will be added after label in UI
        )
        self.comment_after = (
            comment_after  # HTML text that will be added before label in UI
        )
        self.submit = submit
...
default_checkpoint = (
    list_checkpoint_tiles()[0] if len(list_checkpoint_tiles()) > 0 else "model.ckpt"
)
...
def list_builtin_themes():
    files = [
        os.path.splitext(f)[0] for f in os.listdir("javascript") if f.endswith(".css")
    ]
    return files
...
def list_themes():
    ...
    themes = sorted(builtin) + sorted(
        {
            x["id"]
            for x in res
            if x["status"] == "RUNNING" and "test" not in x["id"].lower()
        },
        key=str.casefold,
    )
    return themes
...
def disable_extensions():
    if opts.lora_disable:
        ...
        opts.data["disabled_extensions"] = [
            x for x in opts.disabled_extensions if x != "Lora"
        ]
...
options_templates.update(
    options_section(
        ("sd", "Stable Diffusion"),
        {
            "sd_backend": OptionInfo(
                "original",
                "Stable Diffusion backend",
                gr.Radio,
                lambda: {"choices": ["original", "diffusers"]},
            ),
            "sd_checkpoint_autoload": OptionInfo(
                True, "Stable Diffusion checkpoint autoload on server start"
            ),
            ...
            "sd_model_refiner": OptionInfo(
                "None",
                "Stable Diffusion refiner",
                gr.Dropdown,
                lambda: {"choices": ["None"] + list_checkpoint_tiles()},
                refresh=refresh_checkpoints,
            ),
            "sd_checkpoint_cache": OptionInfo(
                0,
                "Number of cached model checkpoints",
                gr.Slider,
                {"minimum": 0, "maximum": 10, "step": 1},
            ),
            "sd_vae_checkpoint_cache": OptionInfo(
                0,
                "Number of cached VAE checkpoints",
                gr.Slider,
                {"minimum": 0, "maximum": 10, "step": 1},
            ),
            "sd_vae": OptionInfo(
            ...
            "sd_model_dict": OptionInfo(
                "None",
                "Stable Diffusion checkpoint dict",
                gr.Dropdown,
                lambda: {"choices": ["None"] + list_checkpoint_tiles()},
                refresh=refresh_checkpoints,
            ),
            "stream_load": OptionInfo(False, "Load models using stream loading method"),
            "model_reuse_dict": OptionInfo(
                False, "When loading models attempt to reuse previous model dictionary"
            ),
            "prompt_attention": OptionInfo(
                "Full parser",
            ...
            "comma_padding_backtrack": OptionInfo(
                20,
                "Prompt padding for long prompts",
                gr.Slider,
                {"minimum": 0, "maximum": 74, "step": 1},
            ),
            "sd_disable_ckpt": OptionInfo(
                False, "Disallow usage of checkpoints in ckpt format"
            ),
        },
    )
)
    ...
    options_section(
        ("optimizations", "Optimizations"),
        {
            "cross_attention_optimization": OptionInfo(
                cross_attention_optimization_default,
                "Cross-attention optimization method",
                gr.Radio,
                lambda: {"choices": shared_items.list_crossattention()},
            ),
            "cross_attention_options": OptionInfo(
                [],
                "Cross-attention advanced options",
                gr.CheckboxGroup,
                lambda: {
                    "choices": [
                        "xFormers enable flash Attention",
                        "SDP disable memory attention",
                    ]
                },
            ),
            "sub_quad_q_chunk_size": OptionInfo(
                512,
                "Sub-quadratic cross-attention query chunk size",
                gr.Slider,
                {"minimum": 16, "maximum": 8192, "step": 8},
            ),
            "sub_quad_kv_chunk_size": OptionInfo(
                512,
                "Sub-quadratic cross-attention kv chunk size",
                gr.Slider,
                {"minimum": 0, "maximum": 8192, "step": 8},
            ),
            "sub_quad_chunk_threshold": OptionInfo(
                80,
                "Sub-quadratic cross-attention chunking threshold",
                gr.Slider,
            ...
            "token_merging_ratio_hr": OptionInfo(
                0.0,
                "Token merging ratio for hires pass",
                gr.Slider,
                {"minimum": 0.0, "maximum": 0.9, "step": 0.1},
            ),
            "sd_vae_sliced_encode": OptionInfo(
                False, "Enable splitting of hires batch processing"
            ),
        },
    )
)
    ...
    options_section(
        ("cuda", "Compute Settings"),
        {
            ...
            "precision": OptionInfo(
                ...
                "Precision type",
                gr.Radio,
                lambda: {"choices": ["Autocast", "Full"]},
            ),
            "cuda_dtype": OptionInfo(
                "FP32" if sys.platform == "darwin" else "FP16",
                "Device precision type",
                gr.Radio,
                lambda: {"choices": ["FP32", "FP16", "BF16"]},
            ),
            "no_half": OptionInfo(
                False, "Use full precision for model (--no-half)", None, None, None
            ),
            "no_half_vae": OptionInfo(
                False, "Use full precision for VAE (--no-half-vae)"
            ),
            "upcast_sampling": OptionInfo(
                True if sys.platform == "darwin" else False, "Enable upcast sampling"
            ),
            "upcast_attn": OptionInfo(False, "Enable upcast cross attention layer"),
            "cuda_cast_unet": OptionInfo(False, "Use fixed UNet precision"),
            "disable_nan_check": OptionInfo(
                True, "Disable NaN check in produced images/latent spaces"
            ),
            "rollback_vae": OptionInfo(
                False, "Attempt VAE roll back when produced NaN values (experimental)"
            ),
            "opt_channelslast": OptionInfo(
                False, "Use channels last as torch memory format "
            ),
            "cudnn_benchmark": OptionInfo(
                False, "Enable full-depth cuDNN benchmark feature"
            ),
            ...
            "cuda_compile_backend": OptionInfo(
                "none",
                "Model compile backend (experimental)",
                gr.Radio,
                lambda: {
                    "choices": [
                        "none",
            ...
            "cuda_compile_mode": OptionInfo(
                "default",
                "Model compile mode (experimental)",
                gr.Radio,
                lambda: {"choices": ["default", "reduce-overhead", "max-autotune"]},
            ),
            ...
            "ipex_optimize": OptionInfo(
                True if devices.backend == "ipex" else False,
                "Enable IPEX Optimize for Intel GPUs",
            ),
            "directml_memory_provider": OptionInfo(
                default_memory_provider,
                "[DirectML] Memory stats provider",
                gr.Dropdown,
                lambda: {"choices": memory_providers},
            ),
        },
    )
)
...
options_templates.update(
    options_section(
        ("diffusers", "Diffusers Settings"),
        {
            "diffusers_allow_safetensors": OptionInfo(
                True, "Diffusers allow loading from safetensors files"
            ),
            "diffusers_pipeline": OptionInfo(
                pipelines[0],
                "Diffusers pipeline",
                gr.Dropdown,
                lambda: {"choices": pipelines},
            ),
            "diffusers_move_base": OptionInfo(
                False, "Move base model to CPU when using refiner"
            ),
            "diffusers_move_refiner": OptionInfo(
                True, "Move refiner model to CPU when not in use"
            ),
            "diffusers_move_unet": OptionInfo(
                False, "Move UNet to CPU while VAE decoding"
            ),
            "diffusers_extract_ema": OptionInfo(
                True, "Use model EMA weights when possible"
            ),
            "diffusers_generator_device": OptionInfo(
                "default",
                "Generator device",
                gr.Radio,
                lambda: {"choices": ["default", "cpu"]},
            ),
            "diffusers_seq_cpu_offload": OptionInfo(
                False, "Enable sequential CPU offload"
            ),
            "diffusers_model_cpu_offload": OptionInfo(
                False, "Enable model CPU offload"
            ),
            "diffusers_vae_upcast": OptionInfo(
                "default",
                "VAE upcasting",
                gr.Radio,
                lambda: {"choices": ["default", "true", "false"]},
            ),
            "diffusers_vae_slicing": OptionInfo(True, "Enable VAE slicing"),
            "diffusers_vae_tiling": OptionInfo(False, "Enable VAE tiling"),
            "diffusers_attention_slicing": OptionInfo(
                True if devices.backend == "ipex" else False, "Enable attention slicing"
            ),
            "diffusers_model_load_variant": OptionInfo(
                "default",
                "Diffusers model loading variant",
                gr.Radio,
                lambda: {"choices": ["default", "fp32", "fp16"]},
            ),
            "diffusers_vae_load_variant": OptionInfo(
                "default",
                "Diffusers VAE loading variant",
                gr.Radio,
                lambda: {"choices": ["default", "fp32", "fp16"]},
            ),
            "diffusers_lora_loader": OptionInfo(
                "sequential apply",
                "Diffusers LoRA loading variant",
                gr.Radio,
                lambda: {
                    "choices": [
                        "sequential apply",
    ...
    options_section(
        ("system-paths", "System Paths"),
        {
            ...
            "lora_dir": OptionInfo(
                ...
                "Path to directory with Lora network(s)",
            ),
            "lyco_dir": OptionInfo(
                os.path.join(paths.models_path, "LyCORIS"),
                "Path to directory with LyCORIS network(s)",
            ),
            "styles_dir": OptionInfo(
                os.path.join(paths.data_path, "styles.csv"),
                "Path to user-defined styles file",
            ),
            "embeddings_dir": OptionInfo(
                os.path.join(paths.models_path, "embeddings"),
                "Embeddings directory for textual inversion",
            ),
            "hypernetwork_dir": OptionInfo(
                os.path.join(paths.models_path, "hypernetworks"),
                "Hypernetwork directory",
            ),
            "codeformer_models_path": OptionInfo(
                os.path.join(paths.models_path, "Codeformer"),
                "Path to directory with codeformer model file(s)",
            ),
            "gfpgan_models_path": OptionInfo(
                os.path.join(paths.models_path, "GFPGAN"),
                "Path to directory with GFPGAN model file(s)",
            ),
            "esrgan_models_path": OptionInfo(
                os.path.join(paths.models_path, "ESRGAN"),
                "Path to directory with ESRGAN model file(s)",
            ),
            "bsrgan_models_path": OptionInfo(
                os.path.join(paths.models_path, "BSRGAN"),
                "Path to directory with BSRGAN model file(s)",
            ),
            "realesrgan_models_path": OptionInfo(
                os.path.join(paths.models_path, "RealESRGAN"),
                "Path to directory with RealESRGAN model file(s)",
            ),
            "scunet_models_path": OptionInfo(
                os.path.join(paths.models_path, "ScuNET"),
                "Path to directory with ScuNET model file(s)",
            ),
            "swinir_models_path": OptionInfo(
                os.path.join(paths.models_path, "SwinIR"),
                "Path to directory with SwinIR model file(s)",
            ),
            "ldsr_models_path": OptionInfo(
                os.path.join(paths.models_path, "LDSR"),
                "Path to directory with LDSR model file(s)",
            ),
            "clip_models_path": OptionInfo(
    ...
    options_section(
        ("saving-images", "Image Options"),
        {
            ...
            "samples_format": OptionInfo(
                ...
                gr.Dropdown,
                lambda: {"choices": ["jpg", "png", "webp", "tiff", "jp2"]},
            ),
            "image_metadata": OptionInfo(True, "Include metadata in saved images"),
            "image_watermark_enabled": OptionInfo(
                False, "Include watermark in saved images"
            ),
            ...
            "directories_max_prompt_words": OptionInfo(
                ...
                gr.Slider,
                {"minimum": 1, "maximum": 99, "step": 1, **hide_dirs},
            ),
            "save_images_add_number": OptionInfo(
                True, "Add number to filename when saving", component_args=hide_dirs
            ),
            "samples_save_zip": OptionInfo(
                True, "Create zip archive when downloading multiple images"
            ),
            ...
            "grid_extended_filename": OptionInfo(
                True, "Add extended info (seed, prompt) to filename when saving grid"
            ),
            "grid_only_if_multiple": OptionInfo(
                True, "Do not save grids consisting of one picture"
            ),
            "grid_prevent_empty_spots": OptionInfo(
                True, "Prevent empty spots in grid (when set to autodetect)"
            ),
            "n_rows": OptionInfo(
                -1,
                "Grid row count",
            ...
            "save_txt": OptionInfo(
                False, "Create text file next to every image with generation parameters"
            ),
            "save_log_fn": OptionInfo(
                "",
                "Create JSON log file for each saved image",
                component_args=hide_dirs,
            ),
            "save_images_before_highres_fix": OptionInfo(
                False, "Save copy of image before applying highres fix"
            ),
            "save_images_before_refiner": OptionInfo(
                False, "Save copy of image before running refiner"
            ),
            "save_images_before_face_restoration": OptionInfo(
                False, "Save copy of image before doing face restoration"
            ),
            "save_images_before_color_correction": OptionInfo(
                False, "Save copy of image before applying color correction"
            ),
            "save_mask": OptionInfo(
                False, "Save copy of the inpainting greyscale mask"
            ),
            "save_mask_composite": OptionInfo(
                False, "Save copy of inpainting masked composite"
            ),
            ...
            "img_max_size_mp": OptionInfo(
                250, "Maximum allowed image size in megapixels", gr.Number
            ),
            "use_original_name_batch": OptionInfo(
                True, "Use original name for output filename during batch process"
            ),
            "use_upscaler_name_as_suffix": OptionInfo(
                True, "Use upscaler name as filename suffix in the extras tab"
            ),
            ...
            "use_save_to_dirs_for_ui": OptionInfo(
                False, "Save images to a subdirectory when using Save button"
            ),
    ...
    options_section(
        ("image-processing", "Image Processing"),
        {
            "img2img_color_correction": OptionInfo(
                False, "Apply color correction to match original colors"
            ),
            "img2img_fix_steps": OptionInfo(
                False, "For image processing do exact number of steps as specified"
            ),
            "img2img_background_color": OptionInfo(
                "#ffffff",
                "Image transparent color fill",
            ...
            "initial_noise_multiplier": OptionInfo(
                1.0,
                "Noise multiplier for image processing",
                gr.Slider,
                {"minimum": 0.1, "maximum": 1.5, "step": 0.01},
            ),
...
options_templates.update(
    options_section(
        ("saving-paths", "Image Paths"),
        {
            "outdir_samples": OptionInfo(
                "", "Output directory for images", component_args=hide_dirs
            ),
            ...
            "outdir_grids": OptionInfo(
                "", "Output directory for grids", component_args=hide_dirs
            ),
    ...
    options_section(
        ("ui", "User Interface"),
        {
            "gradio_theme": OptionInfo(
                ...
                "UI theme",
                gr.Dropdown,
                lambda: {"choices": list_themes()},
                refresh=refresh_themes,
            ),
            "theme_style": OptionInfo(
                "Auto", "Theme mode", gr.Radio, {"choices": ["Auto", "Dark", "Light"]}
            ),
            "tooltips": OptionInfo(
                "UI Tooltips",
                "UI tooltips",
                gr.Radio,
                {"choices": ["None", "Browser default", "UI tooltips"]},
            ),
            ...
            "disable_weights_auto_swap": OptionInfo(
                True, "Do not change selected model when reading generation parameters"
            ),
            ...
            "ui_tab_reorder": OptionInfo(
                "From Text, From Image, Process Image", "UI tabs order"
            ),
            "ui_scripts_reorder": OptionInfo(
                "Enable Dynamic Thresholding, ControlNet", "UI scripts order"
            ),
        },
    )
)
...
options_templates.update(
    options_section(
        ("live-preview", "Live Previews"),
        {
            ...
            "notification_audio_enable": OptionInfo(
                False, "Play a sound when images are finished generating"
            ),
            "notification_audio_path": OptionInfo(
            ...
            "live_preview_refresh_period": OptionInfo(
                ...
                gr.Slider,
                {"minimum": 0, "maximum": 5000, "step": 25},
            ),
            "logmonitor_show": OptionInfo(True, "Show log view"),
            "logmonitor_refresh_period": OptionInfo(
                5000,
                "Log view update period, in milliseconds",
                gr.Slider,
                {"minimum": 0, "maximum": 30000, "step": 25},
            ),
        },
    )
)
...
options_templates.update(
    options_section(
        ("sampler-params", "Sampler Settings"),
        {
            "show_samplers": OptionInfo(
                ...
                "Show samplers in user interface",
                gr.CheckboxGroup,
                lambda: {
                    "choices": [x.name for x in list_samplers() if x.name != "PLMS"]
                },
            ),
            ...
            "eta_noise_seed_delta": OptionInfo(
                0, "Noise seed delta (eta)", gr.Number, {"precision": 0}
            ),
            "eta_ddim": OptionInfo(
                0.0,
                "Noise multiplier for DDIM (eta)",
                gr.Slider,
                {"minimum": 0.0, "maximum": 1.0, "step": 0.01},
            ),
            "schedulers_solver_order": OptionInfo(
                2,
                "Samplers solver order where applicable",
                gr.Slider,
                {"minimum": 1, "maximum": 5, "step": 1},
            ),
            "schedulers_sep_diffusers": OptionInfo(
                "<h2>Diffusers specific config</h2>", "", gr.HTML
            ),
            "schedulers_prediction_type": OptionInfo(
                "default",
                "Samplers override model prediction type",
                gr.Radio,
                lambda: {"choices": ["default", "epsilon", "sample", "v-prediction"]},
            ),
            "schedulers_use_karras": OptionInfo(
                True, "Samplers should use Karras sigmas where applicable"
            ),
            "schedulers_use_loworder": OptionInfo(
                True,
                "Samplers should use use lower-order solvers in the final steps where applicable",
            ),
            "schedulers_use_thresholding": OptionInfo(
                False, "Samplers should use dynamic thresholding where applicable"
            ),
            "schedulers_dpm_solver": OptionInfo(
                "sde-dpmsolver++",
                "Samplers DPM solver algorithm",
                gr.Radio,
                lambda: {"choices": ["dpmsolver", "dpmsolver++", "sde-dpmsolver++"]},
            ),
            "schedulers_beta_schedule": OptionInfo(
                "default",
                "Samplers override beta schedule",
                gr.Radio,
                lambda: {
                    "choices": [
                        "default",
            ...
            "schedulers_beta_start": OptionInfo(
                0, "Samplers override beta start", gr.Number, {}
            ),
            "schedulers_beta_end": OptionInfo(
                0, "Samplers override beta end", gr.Number, {}
            ),
            "schedulers_sep_kdiffusers": OptionInfo(
                "<h2>K-Diffusion specific config</h2>", "", gr.HTML
            ),
            "always_batch_cond_uncond": OptionInfo(
                False, "Disable conditional batching enabled on low memory systems"
            ),
            "enable_quantization": OptionInfo(
                True, "Enable samplers quantization for sharper and cleaner results"
            ),
            "eta_ancestral": OptionInfo(
                1.0,
                "Noise multiplier for ancestral samplers (eta)",
                gr.Slider,
                {"minimum": 0.0, "maximum": 1.0, "step": 0.01},
            ),
            "s_churn": OptionInfo(
            ...
            "s_min_uncond": OptionInfo(
                0,
                "sigma negative guidance minimum ",
                gr.Slider,
            ...
            "schedulers_sep_compvis": OptionInfo(
                "<h2>CompVis specific config</h2>", "", gr.HTML
            ),
    ...
    options_section(
        ("postprocessing", "Postprocessing"),
        {
            "postprocessing_enable_in_main_ui": OptionInfo(
                [],
                "Enable addtional postprocessing operations",
                ui_components.DropdownMulti,
                lambda: {
    ...
    options_section(
        ("training", "Training"),
        {
            "unload_models_when_training": OptionInfo(
                False, "Move VAE and CLIP to RAM when training if possible"
            ),
            "pin_memory": OptionInfo(True, "Pin training dataset to memory"),
            "save_optimizer_state": OptionInfo(
                False, "Save resumable optimizer state when training"
            ),
            "save_training_settings_to_txt": OptionInfo(
                True, "Save training settings to a text file on training start"
            ),
            "dataset_filename_word_regex": OptionInfo("", "Filename word regex"),
            "dataset_filename_join_string": OptionInfo(" ", "Filename join string"),
            "embeddings_templates_dir": OptionInfo(
                os.path.join(paths.script_path, "train", "templates"),
                "Embeddings train templates directory",
            ),
            ...
            "training_write_csv_every": OptionInfo(
                0, "Save CSV file containing the loss to log directory"
            ),
    ...
    options_section(
        ("interrogate", "Interrogate"),
        {
            ...
            "interrogate_return_ranks": OptionInfo(
                True, "Interrogate: include ranks of model tags matches in results"
            ),
            ...
            "interrogate_clip_dict_limit": OptionInfo(
                2048, "CLIP: maximum number of lines in text file"
            ),
            "interrogate_clip_skip_categories": OptionInfo(
                ["artists", "movements", "flavors"],
            ...
            "deepbooru_sort_alpha": OptionInfo(
                False, "Interrogate: deepbooru sort alphabetically"
            ),
            ...
            "deepbooru_filter_tags": OptionInfo(
                "", "Filter out tags from deepbooru output"
            ),
        },
    )
)
    ...
    options_section(
        ("upscaling", "Upscaling"),
        {
            ...
            "ESRGAN_tile_overlap": OptionInfo(
                8,
                "Tile overlap in pixels for ESRGAN upscalers",
                gr.Slider,
                {"minimum": 0, "maximum": 48, "step": 1},
            ),
            "SCUNET_tile": OptionInfo(
                256,
                "Tile size for SCUNET upscalers",
                gr.Slider,
                {"minimum": 0, "maximum": 512, "step": 16},
            ),
            "SCUNET_tile_overlap": OptionInfo(
                8,
            ...
            "use_old_hires_fix_width_height": OptionInfo(
                False, "Hires fix uses width & height to set final resolution"
            ),
    ...
    options_section(
        ("extra_networks", "Extra Networks"),
        {
            "ui_extra_networks_tab_reorder": OptionInfo(
                "Checkpoints, Lora, LyCORIS, Textual Inversion, Hypernetworks",
                "Extra networks tab order",
            ),
            "extra_networks_card_cover": OptionInfo(
                "sidebar",
                "UI position",
                gr.Radio,
                lambda: {"choices": ["cover", "inline", "sidebar"]},
            ),
            "extra_networks_height": OptionInfo(
                53,
                "UI height (%)",
                gr.Slider,
                {"minimum": 10, "maximum": 100, "step": 1},
            ),
            "extra_networks_sidebar_width": OptionInfo(
                35,
                "UI sidebar width (%)",
                gr.Slider,
                {"minimum": 10, "maximum": 80, "step": 1},
            ),
            "extra_networks_card_lazy": OptionInfo(
                True, "UI card preview lazy loading"
            ),
            "extra_networks_card_size": OptionInfo(
                200,
                "UI card size (px)",
                gr.Slider,
                {"minimum": 20, "maximum": 2000, "step": 1},
            ),
            "extra_networks_card_square": OptionInfo(
                True, "UI disable variable aspect ratio"
            ),
            "extra_networks_card_fit": OptionInfo(
                "cover",
                "UI image contain method",
                gr.Radio,
                lambda: {"choices": ["contain", "cover", "fill"]},
            ),
            "extra_network_skip_indexing": OptionInfo(
                False, "Do not automatically build extra network pages", gr.Checkbox
            ),
            "lyco_patch_lora": OptionInfo(
                False, "Use LyCoris handler for all Lora types", gr.Checkbox
            ),
            "lora_disable": OptionInfo(
                False,
                "Disable built-in Lora handler",
                gr.Checkbox,
                {"visible": True},
                onchange=disable_extensions,
            ),
            "lora_functional": OptionInfo(
                False, "Use Kohya method for handling multiple Loras", gr.Checkbox
            ),
...
class Options:
    ...
    def __setattr__(self, key, value):  # pylint: disable=inconsistent-return-statements
        if self.data is not None:
            if key in self.data or key in self.data_labels:
                ...
                self.data[key] = value
                return
        return super(Options, self).__setattr__(
            key, value
    ...
    def __getattr__(self, item):
        ...
        return super(Options, self).__getattribute__(
            item
        )  # pylint: disable=super-with-arguments
    ...
    def dumpjson(self):
        ...
        metadata = {
            k: {"is_stored": k in self.data, "tab_name": v.section[0]}
...
if cmd_opts.backend is None:
    backend = (
        Backend.DIFFUSERS
        if opts.data.get("sd_backend", "original") == "diffusers"
        else Backend.ORIGINAL
    )
else:
    backend = (
        Backend.DIFFUSERS
        if cmd_opts.backend.lower() == "diffusers"
        else Backend.ORIGINAL
    )
...
cmd_opts.disable_extension_access = (
    cmd_opts.share or cmd_opts.listen or (cmd_opts.server_name or False)
) and not cmd_opts.insecure
(
    devices.device,
...
batch_cond_uncond = opts.always_batch_cond_uncond or not (
...
def reload_gradio_theme(theme_name=None):
    ...
    try:
        req = urllib.request.Request(
            "https://fonts.googleapis.com/css2?family=IBM+Plex+Mono", method="HEAD"
        )
        res = urllib.request.urlopen(
            req, timeout=3.0
        ).status  # pylint: disable=consider-using-with
    except Exception:
        res = 0
    if res != 200:
...
def get_version():
    ...
    if version is None:
        try:
            ...
            res = subprocess.run(
                'git log --pretty=format:"%h %ad" -1 --date=short',
            ...
            ver = (
                res.stdout.decode(encoding="utf8", errors="ignore")
                if len(res.stdout) > 0
                else "  "
            )
...
class Shared(
    ...
    def backend(self):
        return (
            Backend.ORIGINAL
            if opts.data["sd_backend"] == "original"
            else Backend.DIFFUSERS
        )
