...
def list_scripts(scriptdirname, extension):
    ...
    if os.path.exists(base):
        for filename in sorted(os.listdir(base)):
            tmp_list.append(
                ScriptFile(
                    paths.script_path, filename, os.path.join(base, filename), "50"
                )
            )
    ...
    for script in tmp_list:
        if os.path.splitext(script.path)[1].lower() == extension and os.path.isfile(
            script.path
        ):
            if script.basedir == paths.script_path:
                ...
                priority = "1"
            elif script.basedir.startswith(
            ...
            if os.path.isfile(os.path.join(base, "..", ".priority")):
                with open(
                    os.path.join(base, "..", ".priority"), "r", encoding="utf-8"
                ) as f:
                    priority = priority + str(f.read().strip())
            else:
                priority = priority + script.priority
            priority_list.append(
                ScriptFile(script.basedir, script.filename, script.path, priority)
            )
            # log.debug(f'Adding script: {script.basedir} {script.filename} {script.path} {priority}')
    priority_sort = sorted(
        priority_list, key=lambda item: item.priority + item.path.lower(), reverse=False
    )
...
def load_scripts():
    ...
    for scriptfile in scripts_list:
        ...
        try:
            ...
            current_basedir = paths.script_path
            time_load[scriptfile.basedir] = time_load.get(scriptfile.basedir, 0) + (
                time.time() - t0
            )
            sys.path = syspath
    ...
    time_summary = [
        f"{os.path.basename(k)}:{round(v,3)}s"
        for (k, v) in time_load.items()
        if v > 0.05
    ]
...
class ScriptRunner:
    ...
    def initialize_scripts(self, is_img2img):
        ...
        for script_class, path, _basedir, _script_module in (
    ...
    def setup_ui(self):
        ...
        with gr.Group(
            elem_id="scripts_alwayson_img2img"
            if self.is_img2img
            else "scripts_alwayson_txt2img"
        ):
            for script in self.alwayson_scripts:
                ...
                script.group = group
                time_setup[script.title()] = time_setup.get(script.title(), 0) + (
                    time.time() - t0
                )
    ...
    def run(self, p, *args):
        ...
        parsed = p.per_script_args.get(
            script.title(), args[script.args_from : script.args_to]
        )
        t0 = time.time()
        ...
        for script in self.alwayson_scripts:
            try:
                ...
                args = p.per_script_args.get(
                    script.title(), p.script_args[script.args_from : script.args_to]
                )
    ...
    def before_process_batch(self, p, **kwargs):
        ...
        for script in self.alwayson_scripts:
            try:
                ...
                errors.display(
                    e, f"Running script before process batch: {script.filename}"
                )
    ...
    def postprocess_batch(self, p, images, **kwargs):
        ...
        for script in self.alwayson_scripts:
            try:
                ...
                errors.display(
                    e, f"Running script before postprocess batch: {script.filename}"
                )
    ...
    def postprocess_batch_list(self, p, pp: PostprocessBatchListArgs, **kwargs):
        ...
        for script in self.alwayson_scripts:
            try:
                ...
                errors.display(
                    e,
                    f"Running script before postprocess batch list: {script.filename}",
                )
    ...
    def postprocess_image(self, p, pp: PostprocessImageArgs):
        ...
        for script in self.alwayson_scripts:
            try:
                ...
                errors.display(
                    e, f"Running script postprocess image: {script.filename}"
                )
    ...
    def before_component(self, component, **kwargs):
        for script in self.scripts:
            try:
                ...
                time_component[script.title()] = time_component.get(
                    script.title(), 0
                ) + (time.time() - t0)
            except Exception as e:
...
def BlockContext_init(self, *args, **kwargs):
    res = original_BlockContext_init(
        self, *args, **kwargs
    )  # pylint: disable=assignment-from-no-return
    add_classes_to_gradio_component(self)
    return res
