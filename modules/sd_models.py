...
class CheckpointInfo:
    def __init__(self, filename):
        ...
        if os.path.isfile(abspath):  # ckpt or safetensor
            if shared.opts.ckpt_dir is not None and abspath.startswith(
                shared.opts.ckpt_dir
            ):
        ...
        self.title = (
            self.name if self.shorthash is None else f"{self.name} [{self.shorthash}]"
        )
        self.ids = [
            self.hash,
            self.model_name,
...
def list_models():
    ...
    ext_filter = (
        [".safetensors"] if shared.opts.sd_disable_ckpt else [".ckpt", ".safetensors"]
    )
    model_list = []
    if (
        shared.backend == shared.Backend.ORIGINAL
        or shared.opts.diffusers_allow_safetensors
    ):
        model_list += modelloader.load_models(
            model_path=model_path,
            model_url=None,
            command_path=shared.opts.ckpt_dir,
            ext_filter=ext_filter,
            download_name=None,
            ext_blacklist=[".vae.ckpt", ".vae.safetensors"],
        )
    if shared.backend == shared.Backend.DIFFUSERS:
        model_list += modelloader.load_diffusers_models(
            model_path=os.path.join(models_path, "Diffusers"),
            command_path=shared.opts.diffusers_dir,
        )
    ...
    if shared.cmd_opts.ckpt is not None:
        if (
            not os.path.exists(shared.cmd_opts.ckpt)
            and shared.backend == shared.Backend.ORIGINAL
        ):
            if shared.cmd_opts.ckpt.lower() != "none":
                shared.log.warning(
                    f"Requested checkpoint not found: {shared.cmd_opts.ckpt}"
                )
        else:
        ...
        shared.cmd_opts.ckpt != shared.default_sd_model_file
        and shared.cmd_opts.ckpt is not None
    ):
        ...
        if not shared.cmd_opts.no_download:
            ...
            if key.lower().startswith("y"):
                if shared.backend == shared.Backend.ORIGINAL:
                    ...
                    modelloader.download_diffusers_model(
                        default_model_id, shared.opts.diffusers_dir
                    )
                    model_list = modelloader.load_diffusers_models(
                        model_path=os.path.join(models_path, "Diffusers"),
                        command_path=shared.opts.diffusers_dir,
...
def update_model_hashes():
    ...
    shared.log.info(
        f"Models list: short hash missing for {len(lst)} out of {len(checkpoints_list)} models"
    )
    for ckpt in lst:
        ckpt.hash = model_hash(ckpt.filename)
        txt.append(f"Calculated short hash: <b>{ckpt.title}</b> {ckpt.hash}")
    txt.append(
        f"Updated short hashes for <b>{len(lst)}</b> out of <b>{len(checkpoints_list)}</b> models"
    )
    lst = [
        ckpt
        for ckpt in checkpoints_list.values()
        if ckpt.sha256 is None or ckpt.shorthash is None
    ]
    shared.log.info(
        f"Models list: full hash missing for {len(lst)} out of {len(checkpoints_list)} models"
    )
    for ckpt in lst:
    ...
    txt.append(
        f"Updated full hashes for <b>{len(lst)}</b> out of <b>{len(checkpoints_list)}</b> models"
    )
    txt = "<br>".join(txt)
    return txt
...
def get_closet_checkpoint_match(search_string):
    ...
    found = sorted(
        [
            ...
            if search_string.split(" ")[0] in info.title
        ],
        key=lambda x: len(x.title),
    )
    if found:
...
def select_checkpoint(op="model"):
    ...
    if checkpoint_info is not None:
        shared.log.debug(
            f"Select checkpoint: {op} {checkpoint_info.title if checkpoint_info is not None else None}"
        )
        return checkpoint_info
    if len(checkpoints_list) == 0:
        shared.log.error("Cannot run without a checkpoint")
        shared.log.error(
            "Use --ckpt <path-to-checkpoint> to force using existing checkpoint"
        )
        return None
    ...
    shared.log.debug(
        f"Select checkpoint: {checkpoint_info.title if checkpoint_info is not None else None}"
    )
    return checkpoint_info
...
def read_state_dict(
    checkpoint_file, map_location=None
    ...
    try:
        ...
        with progress.open(
            checkpoint_file,
            "rb",
            description=f"Loading weights: [cyan]{checkpoint_file}",
            auto_refresh=True,
        ) as f:
...
def load_model_weights(
    model: torch.nn.Module, checkpoint_info: CheckpointInfo, state_dict, timer
):
...
def repair_config(sd_config):
    ...
    if (
        "noise_aug_config" in sd_config.model.params
        and "clip_stats_path" in sd_config.model.params.noise_aug_config.params
    ):
            ...
            with self.lock:
                try:
                    if shared.backend == shared.Backend.ORIGINAL:
                        ...
                        shared.log.error(
                            f"Unknown Stable Diffusion backend: {shared.backend}"
                        )
                    self.initial = False
                except Exception as e:
...
def load_diffuser(
    checkpoint_info=None, already_loaded_state_dict=None, timer=None, op="model"
):  # pylint: disable=unused-argument
    ...
    if (
        shared.opts.data.get("sd_model_checkpoint", "") == "model.ckpt"
        or shared.opts.data.get("sd_model_checkpoint", "") == ""
    ):
        shared.opts.data["sd_model_checkpoint"] = "runwayml/stable-diffusion-v1-5"

    if op == "model" or op == "dict":
        if (
            (model_data.sd_model is not None)
            and (checkpoint_info is not None)
            and (checkpoint_info.hash == model_data.sd_model.sd_checkpoint_info.hash)
        ):  # trying to load the same model
            return
    else:
        if (
            (model_data.sd_refiner is not None)
            and (checkpoint_info is not None)
            and (checkpoint_info.hash == model_data.sd_refiner.sd_checkpoint_info.hash)
        ):  # trying to load the same model
            return
    ...
    try:
        if shared.cmd_opts.ckpt is not None and model_data.initial:  # initial load\
            ...
            if model_name is not None:
                ...
                try:
                    shared.log.debug(
                        f"Diffusers load {op} config: {diffusers_load_config}"
                    )
                    sd_model = diffusers.DiffusionPipeline.from_pretrained(
                        model_file, **diffusers_load_config
                    )
                except Exception as e:
                    shared.log.error(
                        f"Diffusers failed loading model: {model_file} {e}"
                    )
        ...
        if sd_model is None:
            ...
            if not os.path.isfile(checkpoint_info.path):
                try:
                    ...
                    sd_model = diffusers.DiffusionPipeline.from_pretrained(
                        checkpoint_info.path, **diffusers_load_config
                    )
                except Exception as e:
                    shared.log.error(
                        f"Diffusers {op} failed loading model: {checkpoint_info.path} {e}"
                    )
            else:
                ...
                try:
                    if shared.opts.diffusers_pipeline == shared.pipelines[0]:
                        ...
                        shared.log.error(
                            f"Diffusers {op} unknown pipeline: {shared.opts.diffusers_pipeline}"
                        )
                except Exception as e:
                    shared.log.error(
                        f"Diffusers {op} failed initializing pipeline: {shared.opts.diffusers_pipeline} {e}"
                    )
                    return
                try:
                    if hasattr(pipeline, "from_single_file"):
                        diffusers_load_config["use_safetensors"] = True
                        sd_model = pipeline.from_single_file(
                            checkpoint_info.path, **diffusers_load_config
                        )
                    elif hasattr(pipeline, "from_ckpt"):
                        sd_model = pipeline.from_ckpt(
                            checkpoint_info.path, **diffusers_load_config
                        )
                    else:
                        shared.log.error(
                            f"Diffusers {op} cannot load safetensor model: {checkpoint_info.path} {shared.opts.diffusers_pipeline}"
                        )
                        return
                    if sd_model is not None:
                        shared.log.debug(
                            f"Diffusers {op}: pipeline={sd_model.__class__.__name__}"
                        )  # pylint: disable=protected-access
                except Exception as e:
                    shared.log.error(
                        f"Diffusers failed loading model using pipeline: {checkpoint_info.path} {shared.opts.diffusers_pipeline} {e}"
                    )
                    return
        ...
        if (shared.opts.diffusers_model_cpu_offload or shared.cmd_opts.medvram) and (
            shared.opts.diffusers_seq_cpu_offload or shared.cmd_opts.lowvram
        ):
            shared.log.warning(
                f"Diffusers {op}: Model CPU offload (--medvram) and Sequential CPU offload (--lowvram) are not compatible"
            )
        ...
        if hasattr(sd_model, "enable_model_cpu_offload"):
            if (
                shared.cmd_opts.medvram and devices.backend != "directml"
            ) or shared.opts.diffusers_model_cpu_offload:
        ...
        if hasattr(sd_model, "vae"):
            if shared.opts.diffusers_vae_upcast != "default":
                if shared.opts.diffusers_vae_upcast == "true":
                    ...
                    sd_model.vae.config.force_upcast = False
            shared.log.debug(
                f'Diffusers {op} VAE: name={sd_model.vae.config.get("_name_or_path", "default")} upcast={sd_model.vae.config.get("force_upcast", None)}'
            )
        if shared.opts.cross_attention_optimization == "xFormers" and hasattr(
        ...
        if (
            shared.opts.cuda_compile or shared.opts.ipex_optimize
            ...
            if op == "refiner" and not sd_model.has_accelerate:
                ...
                refiner_enough_vram = (
                    free_vram >= 7
                    if "StableDiffusionXL" in sd_model.__class__.__name__
                    else 3
                )
                if not shared.opts.diffusers_move_base and refiner_enough_vram:
                    ...
                    if not refiner_enough_vram and not (
                        shared.opts.diffusers_move_base
                        and shared.opts.diffusers_move_refiner
                    ):
                        shared.log.warning(
                            f"Insufficient GPU memory, using system memory as fallback: free={free_vram} GB"
                        )
            ...
            try:
                if shared.opts.ipex_optimize:
                    ...
                    sd_model.unet = torch.xpu.optimize(
                        sd_model.unet,
            ...
            try:
                if shared.opts.cuda_compile:
                    shared.log.info(
                        f"Compiling pipeline={sd_model.__class__.__name__} shape={8 * sd_model.unet.config.sample_size} mode={shared.opts.cuda_compile_backend}"
                    )
                    import torch._dynamo  # pylint: disable=unused-import,redefined-outer-name

                    log_level = (
                        logging.WARNING
                        if shared.opts.cuda_compile_verbose
                        else logging.CRITICAL
                    )  # pylint: disable=protected-access
                    if hasattr(torch, "_logging"):
                        torch._logging.set_logs(
                            dynamo=log_level, aot=log_level, inductor=log_level
                        )  # pylint: disable=protected-access
                    torch._dynamo.config.verbose = (
                        shared.opts.cuda_compile_verbose
                    )  # pylint: disable=protected-access
                    torch._dynamo.config.suppress_errors = (
                        shared.opts.cuda_compile_errors
                    )  # pylint: disable=protected-access
                    sd_model.unet = torch.compile(
                        sd_model.unet,
        ...
        sd_model.sd_checkpoint_info = (
            checkpoint_info  # pylint: disable=attribute-defined-outside-init
        )
        sd_model.sd_model_checkpoint = (
            checkpoint_info.filename
        )  # pylint: disable=attribute-defined-outside-init
        sd_model.sd_model_hash = (
            checkpoint_info.hash
        )  # pylint: disable=attribute-defined-outside-init
        if hasattr(sd_model, "set_progress_bar_config"):
            sd_model.set_progress_bar_config(
                bar_format="Progress {rate_fmt}{postfix} {bar} {percentage:3.0f}% {n_fmt}/{total_fmt} {elapsed} {remaining}",
                ncols=80,
                colour="#327fba",
            )
        if (
            op == "refiner"
            and shared.opts.diffusers_move_refiner
            and not sd_model.has_accelerate
        ):
...
def set_diffuser_pipe(pipe, new_pipe_type):
    ...
    shared.log.info(
        f"Pipeline class changed from {pipe.__class__.__name__} to {new_pipe.__class__.__name__}"
    )
...
def get_diffusers_task(pipe: diffusers.DiffusionPipeline) -> DiffusersTaskType:
    if (
        pipe.__class__
        in diffusers.pipelines.auto_pipeline.AUTO_IMAGE2IMAGE_PIPELINES_MAPPING.values()
    ):
        return DiffusersTaskType.IMAGE_2_IMAGE
    elif (
        pipe.__class__
        in diffusers.pipelines.auto_pipeline.AUTO_INPAINT_PIPELINES_MAPPING.values()
    ):
...
def load_model(
    ...
    if checkpoint_info is None:
        return
    if op == "model" or op == "dict":
        if model_data.sd_model is not None and (
            checkpoint_info.hash == model_data.sd_model.sd_checkpoint_info.hash
        ):  # trying to load the same model
            return
    else:
        if model_data.sd_refiner is not None and (
            checkpoint_info.hash == model_data.sd_refiner.sd_checkpoint_info.hash
        ):  # trying to load the same model
            return
    shared.log.debug(
        f"Load {op}: name={checkpoint_info.filename} dict={already_loaded_state_dict is not None}"
    )
    if timer is None:
        timer = Timer()
    ...
    if state_dict is None or checkpoint_config is None:
        ...
        if current_checkpoint_info is not None:
            shared.log.info(
                f"Restoring previous checkpoint: {current_checkpoint_info.filename}"
            )
    ...
    devices.torch_gc(force=True)
    shared.log.info(
        f"Model load finished: {memory_stats()} cached={len(checkpoints_loaded.keys())}"
    )
...
def reload_model_weights(sd_model=None, info=None, reuse_dict=False, op="model"):
    ...
    checkpoint_info = info or select_checkpoint(
        op=op
    )  # are we selecting model or dictionary
    next_checkpoint_info = (
        info or select_checkpoint(op="dict" if load_dict else "model")
        if load_dict
        else None
    )
    if checkpoint_info is None:
    ...
    if load_dict:
        shared.log.debug(
            f"Model dict: existing={sd_model is not None} target={checkpoint_info.filename} info={info}"
        )
    else:
        model_data.sd_dict = "None"
        shared.log.debug(
            f"Load model weights: existing={sd_model is not None} target={checkpoint_info.filename} info={info}"
        )
    if sd_model is None:
        sd_model = (
            model_data.sd_model
            if op == "model" or op == "dict"
            else model_data.sd_refiner
        )
    if sd_model is None:  # previous model load failed
        ...
        if (
            current_checkpoint_info is not None
            and checkpoint_info is not None
            and current_checkpoint_info.filename == checkpoint_info.filename
        ):
            return
        if not sd_model.has_accelerate:
            if shared.cmd_opts.lowvram or shared.cmd_opts.medvram:
                ...
                sd_model.to(devices.cpu)
    if (
        reuse_dict or (shared.opts.model_reuse_dict and sd_model is not None)
    ) and not sd_model.has_accelerate:
    ...
    if sd_model is None or checkpoint_config != sd_model.used_config:
        ...
        if load_dict and next_checkpoint_info is not None:
            ...
            reload_model_weights(
                reuse_dict=True
            )  # ok we loaded dict now lets redo and load model on top of it
        return (
            model_data.sd_model
            if op == "model" or op == "dict"
            else model_data.sd_refiner
    ...
    try:
        ...
        timer.record("callbacks")
        if (
            not shared.cmd_opts.lowvram
            and not shared.cmd_opts.medvram
            and not sd_model.has_accelerate
        ):
            sd_model.to(devices.device)
            timer.record("device")
