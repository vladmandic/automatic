    ...
    with gr.Row(id="models_tab", elem_id="models_tab"):
        ...
        with gr.Column(elem_id="models_input_container", scale=3):
            ...
            with gr.Tab(label="Convert"):
                with gr.Row():
                    model_name = gr.Dropdown(
                        sd_models.checkpoint_tiles(), label="Original model"
                    )
                ...
                with gr.Row():
                    show_extra_options = gr.Checkbox(
                        label="Show extra options", value=False
                    )
                ...
                with gr.Row(visible=False) as extra_options:
                    ...
                    text_encoder_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="text encoder"
                    )
                    vae_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="vae"
                    )
                    others_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="others"
                    )
                    ...
                    with gr.Column(variant="compact"):
                        ...
                        with FormRow():
                            ...
                            primary_model_name = gr.Dropdown(
                                sd_model_choices(), label="Primary model", value="None"
                            )
                            create_refresh_button(
                                primary_model_name,
                                sd_models.list_models,
                            ...
                            tertiary_model_name = gr.Dropdown(
                                sd_model_choices(), label="Tertiary model", value="None"
                            )
                            create_refresh_button(
                                tertiary_model_name,
                                sd_models.list_models,
                        ...
                        with FormRow():
                            ...
                            interp_amount = gr.Slider(
                                ...
                                maximum=1.0,
                                step=0.05,
                                label="Interpolation ratio from Primary to Secondary",
                                value=0.5,
                            )
                        with FormRow():
                        ...
                        with FormRow():
                            config_source = gr.Radio(
                                choices=["Primary", "Secondary", "Tertiary", "None"],
                                value="Primary",
                        ...
                        with FormRow():
                            discard_weights = gr.Textbox(
                                value="", label="Discard weights with matching name"
                            )
                        with FormRow():
            ...
            with gr.Tab(label="Validate"):
                ...
                with gr.Row():
                    model_list_btn = gr.Button(
                        value="List model details", variant="primary"
                    )
                    model_checkhash_btn = gr.Button(
                        value="Calculate hash for all models (may take a long time)",
                        variant="primary",
                    )
                    model_checkhash_btn.click(
                        fn=sd_models.update_model_hashes,
                        inputs=[],
                        outputs=[models_outcome],
                    )
                with gr.Row():
                ...
                def list_models():
                    ...
                    for m in sd_models.checkpoints_list.values():
                        try:
                            ...
                            m_name = m.name.replace(".ckpt", "").replace(
                                ".safetensors", ""
                            )
                ...
                model_list_btn.click(
                    fn=list_models, inputs=[], outputs=[model_table, models_outcome]
                )
            ...
            with gr.Tab(label="Huggingface"):
                ...
                def hf_download_model(hub_id: str, token, variant, revision, mirror):
                    ...
                    try:
                        ...
                        log.error(
                            f"Diffuser model downloaded error: model={hub_id} {e}"
                        )
                ...
                with gr.Column(scale=6):
                    ...
                    with gr.Row():
                        ...
                        with gr.Column(scale=1):
                            with gr.Row():
                                hf_variant = gr.Textbox(
                                    opts.cuda_dtype.lower(),
                                    label="Specify model variant",
                                    placeholder="",
                                )
                                hf_revision = gr.Textbox(
                                    "", label="Specify model revision", placeholder=""
                                )
                    with gr.Row():
                        hf_token = gr.Textbox(
                            "",
                        ...
                        hf_mirror = gr.Textbox(
                            "",
                            label="Huggingface mirror",
                            placeholder="optional mirror site for downloads",
                        )
                with gr.Column(scale=1):
                    hf_download_model_btn = gr.Button(
                        value="Download model", variant="primary"
                    )
                ...
                hf_search_text.submit(
                    fn=hf_search, inputs=[hf_search_text], outputs=[hf_results]
                )
                hf_search_btn.click(
                    fn=hf_search, inputs=[hf_search_text], outputs=[hf_results]
                )
                hf_results.select(
                    fn=hf_select, inputs=[hf_results], outputs=[hf_selected]
                )
                hf_download_model_btn.click(
                    fn=hf_download_model,
