...
def torch_gc(force=False):
    ...
    if used > 95:
        ...
        force = True
        if backend == "directml":
            practical_used = round(
                100 * torch.cuda.memory_allocated() / (1 << 30) / gpu.get("total", 1)
            )
    ...
    if cuda_ok or backend == "ipex":
        try:
            ...
            pass
    shared.log.debug(
        f"gc: collected={collected} device={torch.device(get_optimal_device_name())} {memstats.memory_stats()}"
    )
...
def test_fp16():
    if shared.cmd_opts.experimental:
        return True
    try:
        x = torch.tensor([[1.5, 0.0, 0.0, 0.0]]).to(device).half()
        layerNorm = torch.nn.LayerNorm(
            4, eps=0.00001, elementwise_affine=True, dtype=torch.float16, device=device
        )
        _y = layerNorm(x)
...
def set_cuda_params():
    ...
    shared.log.debug(
        f"Desired Torch parameters: dtype={shared.opts.cuda_dtype} no-half={shared.opts.no_half} no-half-vae={shared.opts.no_half_vae} upscast={shared.opts.upcast_sampling}"
    )
    shared.log.info(
        f"Setting Torch parameters: dtype={dtype} vae={dtype_vae} unet={dtype_unet}"
    )
...
def without_autocast(disable=False):
    ...
    if shared.cmd_opts.use_directml:
        return (
            torch.dml.amp.autocast(enabled=False)
            if torch.is_autocast_enabled()
            else contextlib.nullcontext()
        )  # pylint: disable=unexpected-keyword-arg
    if backend == "ipex":
        return (
            torch.xpu.amp.autocast(enabled=False)
            if torch.is_autocast_enabled()
            else contextlib.nullcontext()
        )
    if cuda_ok:
