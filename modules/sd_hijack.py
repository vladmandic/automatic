...
def apply_optimizations():
    ...
    if devices.device == torch.device("cpu"):
        if opts.cross_attention_optimization == "Scaled-Dot-Product":
            shared.log.warning(
                "Scaled dot product cross attention is not available on CPU"
            )
            can_use_sdp = False
    ...
    if (
        opts.cross_attention_optimization
    ...
    if (
        can_use_sdp
        and opts.cross_attention_optimization == "Scaled-Dot-Product"
        and "SDP disable memory attention" in opts.cross_attention_options
    ):
        shared.log.info(
...
def weighted_loss(sd_model, pred, target, mean=True):
    ...
    loss = sd_model._old_get_loss(
        pred, target, mean=False
...
class StableDiffusionModelHijack:
    ...
    def hijack(self, m):
        ...
        if (
            opts.cuda_compile
            and opts.cuda_compile_backend != "none"
            ...
            try:
                ...
                shared.log.info(
                    f"Compiling pipeline={m.model.__class__.__name__} mode={opts.cuda_compile_backend}"
                )
                import torch._dynamo  # pylint: disable=unused-import,redefined-outer-name

                log_level = (
                    logging.WARNING if opts.cuda_compile_verbose else logging.CRITICAL
                )  # pylint: disable=protected-access
                ...
                torch._dynamo.config.verbose = (
                    opts.cuda_compile_verbose
                )  # pylint: disable=protected-access
                torch._dynamo.config.suppress_errors = (
                    opts.cuda_compile_errors
                )  # pylint: disable=protected-access
                ...
                m.model = torch.compile(
                    m.model,
                    mode=opts.cuda_compile_mode,
                    backend=opts.cuda_compile_backend,
                    fullgraph=opts.cuda_compile_fullgraph,
                    dynamic=False,
                )
...
if not devices.backend == "ipex":
    ldm.modules.distributions.distributions.DiagonalGaussianDistribution.sample = (
        lambda self: self.mean.to(self.parameters.dtype)
        + self.std.to(self.parameters.dtype)
        * torch.randn(self.mean.shape, dtype=self.parameters.dtype).to(
