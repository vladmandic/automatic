...
from .sub_quadratic_attention import (
...
def split_cross_attention_forward_v1(
...
def einsum_op_dml(q, k, v):
    ...
    return einsum_op_tensor_mem(
        q, k, v, (mem_reserved - mem_active) if mem_reserved > mem_active else 1
    )
...
def split_cross_attention_forward_invokeAI(
    self, x, context=None, mask=None
):  # pylint: disable=unused-argument
    h = self.heads
...
def sub_quad_attention_forward(self, x, context=None, mask=None):
    ...
    x = sub_quad_attention(
        ...
        k,
        v,
        q_chunk_size=shared.opts.sub_quad_q_chunk_size,
        kv_chunk_size=shared.opts.sub_quad_kv_chunk_size,
        chunk_threshold=shared.opts.sub_quad_chunk_threshold,
...
def sdp_attnblock_forward(self, x):
    ...
    out = out.to(dtype)
    out = rearrange(
        out, "b a (h w) c -> (b a) c h w", h=h
    )  # remove the one attention head dimension `a`
    out = self.proj_out(out)
    return x + out
