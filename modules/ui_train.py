...
def create_ui(txt2img_preview_params):
    ...
    with gr.Row(id="train_tab", elem_id="train_tab"):
        with gr.Column(elem_id="train_output_container", scale=1):
            ...
            gr.Gallery(label="Output", show_label=False, elem_id="train_gallery").style(
                columns=1
            )
        ...
        with gr.Column(elem_id="train_input_container", scale=3):
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(
                    label="Preprocess images", id="preprocess_images"
                ) as tab_preprocess:
                    tab_preprocess.select(
                        fn=lambda x: train_tab_change("pp"),
                        inputs=[],
                        outputs=[action_pp, action_ti, action_hn],
                    )
                    process_src = gr.Textbox(label="Source directory")
                    process_dst = gr.Textbox(label="Destination directory")
                    with gr.Row():
                        process_width = gr.Slider(
                            minimum=64, maximum=2048, step=8, label="Width", value=512
                        )
                        process_height = gr.Slider(
                            minimum=64, maximum=2048, step=8, label="Height", value=512
                        )
                    preprocess_txt_action = gr.Dropdown(
                        label="Existing caption text action",
                        value="ignore",
                    ...
                    with gr.Box():
                        ...
                        process_keep_channels = gr.Checkbox(
                            label="Keep original image channels"
                        )
                        ...
                        process_caption_deepbooru = gr.Checkbox(
                            label="Create Deepbooru captions"
                        )
                    ...
                    process_stop.click(
                        fn=lambda: shared.state.interrupt(), inputs=[], outputs=[]
                    )
                    process_run.click(
                        fn=wrap_gradio_gpu_call(
                            modules.textual_inversion.ui.preprocess,
                ...
                with gr.Tab(
                    label="Train embedding", id="train_embedding_tab"
                ) as tab_ti:
                    tab_ti.select(
                        fn=lambda x: train_tab_change("ti"),
                        inputs=[],
                        outputs=[action_pp, action_ti, action_hn],
                    )
                    ...
                    with FormRow():
                        ...
                        with gr.Column():
                            ...
                            ti_init_text = gr.Textbox(
                                label="Initialization text", value="*"
                            )
                            ti_vectors = gr.Slider(
                            ...
                            ti_overwrite = gr.Checkbox(
                                value=False, label="Overwrite Old Embedding"
                            )
                            with gr.Row():
                                ti_create = gr.Button(
                                    value="Create embedding", variant="secondary"
                                )

                    with gr.Box():
                        gr.Markdown("## Training parameters")
                        ti_learn_rate = gr.Textbox(
                            label="Embedding Learning rate",
                            placeholder="Embedding Learning rate",
                            value="0.005",
                        )
                        with FormRow():
                            ti_clip_grad_mode = gr.Dropdown(
                                value="disabled",
                                label="Gradient Clipping",
                                choices=["disabled", "value", "norm"],
                            )
                            ti_clip_grad_value = gr.Number(
                                label="Gradient clip value", value=0.1
                            )
                        ti_batch_size = gr.Number(
                            label="Batch size", value=1, precision=0
                        )
                        ti_gradient_step = gr.Number(
                    ...
                    with gr.Box():
                        ...
                        ti_dataset_directory = gr.Textbox(
                            label="Dataset directory",
                            placeholder="Path to directory with input images",
                        )
                        with FormRow():
                            ti_varsize = gr.Checkbox(
                                label="Do not resize images", value=False
                            )
                            ti_width = gr.Slider(
                                minimum=64,
                                maximum=2048,
                        ...
                        ti_use_weight = gr.Checkbox(
                            label="Use PNG alpha channel as loss weight", value=False
                        )

                    with gr.Box():
                        gr.Markdown("## Dataset processing")
                        with FormRow():
                            ti_template = gr.Dropdown(
                        ...
                        ti_latent_sampling_method = gr.Radio(
                    ...
                    with gr.Box():
                        ...
                        with FormRow():
                            ti_create_every = gr.Number(
                                label="Create interim images", value=500, precision=0
                            )
                            ti_save_every = gr.Number(
                                label="Create interim embeddings",
                                value=500,
                                precision=0,
                            )
                        ti_save_image_with_stored_embedding = gr.Checkbox(
                            label="Save images with embedding in PNG chunks", value=True
                        )
                        ti_preview_from_txt2img = gr.Checkbox(
                            label="Use current settings for previews", value=False
                        )
                        ti_log_directory = gr.Textbox(
                            label="Log directory",
                            placeholder="Defaults to train/log/embedding",
                            value="",
                        )
                ...
                with gr.Tab(
                    label="Train hypernetwork", id="train_hypernetwork_tab"
                ) as tab_hn:
                    tab_hn.select(
                        fn=lambda x: train_tab_change("hn"),
                        inputs=[],
                        outputs=[action_pp, action_ti, action_hn],
                    )
                    gr.Markdown(
                    ...
                    with FormRow():
                        ...
                        with gr.Column():
                            ...
                            hn_new_add_layer_norm = gr.Checkbox(
                                label="Add layer normalization"
                            )
                            hn_new_use_dropout = gr.Checkbox(label="Use dropout")
                            hn_new_dropout_structure = gr.Textbox(
                                "0, 0, 0",
                                label="Enter hypernetwork Dropout structure",
                                placeholder="1st and last digit must be 0 and values should be between 0 and 1. ex:'0, 0.01, 0'",
                            )
                            hn_overwrite = gr.Checkbox(
                                value=False, label="Overwrite Old Hypernetwork"
                            )
                            with gr.Row():
                                hn_create = gr.Button(
                                    value="Create hypernetwork", variant="secondary"
                                )

                    with gr.Box():
                        gr.Markdown("## Training parameters")
                        hn_learn_rate = gr.Textbox(
                            label="Hypernetwork Learning rate",
                            placeholder="Hypernetwork Learning rate",
                            value="0.00001",
                        )
                        with FormRow():
                            hn_clip_grad_mode = gr.Dropdown(
                                value="disabled",
                                label="Gradient Clipping",
                                choices=["disabled", "value", "norm"],
                            )
                            hn_clip_grad_value = gr.Number(
                                label="Gradient clip value", value=0.1
                            )
                        hn_batch_size = gr.Number(
                            label="Batch size", value=1, precision=0
                        )
                        hn_gradient_step = gr.Number(
                            label="Gradient accumulation steps", value=1, precision=0
                        )
                    ...
                    with gr.Box():
                        gr.Markdown("## Training images")
                        hn_dataset_directory = gr.Textbox(
                            label="Dataset directory",
                            placeholder="Path to directory with input images",
                        )
                        with FormRow():
                            hn_varsize = gr.Checkbox(
                                label="Do not resize images", value=False
                            )
                            hn_width = gr.Slider(
                                minimum=64,
                    ...
                    with gr.Box():
                        ...
                        hn_shuffle_tags = gr.Checkbox(
                            label="Shuffle tags by ',' when creating prompts.",
                            value=False,
                        )
                        hn_tag_drop_out = gr.Slider(
                            minimum=0,
                            maximum=1,
                            step=0.1,
                            label="Drop out tags when creating prompts",
                        ...
                        hn_latent_sampling_method = gr.Radio(
                            label="Choose latent sampling method",
                            value="once",
                            choices=["once", "deterministic", "random"],
                        )
                    ...
                    with gr.Box():
                        gr.Markdown("## Training outputs")
                        with FormRow():
                            hn_create_every = gr.Number(
                                label="Create interim images", value=500, precision=0
                            )
                            hn_save_every = gr.Number(
                                label="Create interim hypernetworks",
                                value=500,
                                precision=0,
                            )
                        hn_preview_from_txt2img = gr.Checkbox(
                            label="Use current settings for previews", value=False
                        )
                        hn_log_directory = gr.Textbox(
                            label="Log directory",
                            placeholder="Path to directory where to write outputs",
                            value=f"{os.path.join('cmd_opts.data_dir', 'train/log/embeddings')}",
                        )
