...
class UniPC:
    ...
    def get_orders_and_timesteps_for_singlestep_solver(
        ...
        if skip_type == "logSNR":
            ...
            timesteps_outer = get_time_steps(
                self.noise_schedule, skip_type, t_T, t_0, K, device
            )
        else:
            timesteps_outer = get_time_steps(
                self.noise_schedule, skip_type, t_T, t_0, steps, device
            )[
                torch.cumsum(
                    torch.tensor(
                        [
                ...
                with torch.no_grad():
                    ...
                    for init_order in range(1, order):
                        ...
                        progress.update(
                            task,
                            advance=1,
                            description=f"Progress {round(len(vec_t) * init_order / (time.time() - t), 2)}it/s",
                        )
                    ...
                    for step in range(order, steps + 1):
                        ...
                        progress.update(
                            task,
                            advance=1,
                            description=f"Progress {round(len(vec_t) * step / (time.time() - t), 2)}it/s",
                        )
        else:
            raise NotImplementedError()
        if denoise_to_zero:
