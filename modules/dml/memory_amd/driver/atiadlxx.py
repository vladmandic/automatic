...
class ATIADLxx(object):
    ...
    def __init__(self):
        ...
        ADL2_Adapter_AdapterInfo_Get(
            self.context,
            C.cast(AdapterInfoArray, LPAdapterInfo),
            C.sizeof(AdapterInfoArray),
        )
        self.devices = []
        busNumbers = []
    ...
    def get_memory_info2(self, adapterIndex: int) -> ADLMemoryInfo2:
        ...
        if (
            ADL2_Adapter_MemoryInfo2_Get(self.context, adapterIndex, C.byref(info))
            != ADL_OK
        ):
    ...
    def get_dedicated_vram_usage(self, index: int) -> int:
        ...
        if (
            ADL2_Adapter_DedicatedVRAMUsage_Get(
                self.context, self.devices[index].iAdapterIndex, C.byref(usage)
            )
            != ADL_OK
        ):
