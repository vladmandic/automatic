...
def delete_files(js_data, images, _html_info, index):
    ...
    return images, plaintext_to_html(
        f"Deleted: {filenames[0] if len(filenames) > 0 else 'none'}"
    )
...
def save_files(js_data, images, html_info, index):
    ...
    if (
        index > -1
        and shared.opts.save_selected_only
        and (index >= p.index_of_first_image)
    ):  # ensures we are looking at a specific non-grid picture, and we have save_selected_only # pylint: disable=no-member
        images = [images[index]]
        start_index = index
    filenames = []
    fullfns = []
    for image_index, filedata in enumerate(images, start_index):
        ...
        i = (
            0 if is_grid else (image_index - p.index_of_first_image)
        ...
        if (
            "name" in filedata
            and ("tmp" not in filedata["name"])
            and os.path.isfile(filedata["name"])
        ):
            fullfn = filedata["name"]
            filenames.append(os.path.basename(fullfn))
            fullfns.append(fullfn)
            ...
            if shared.opts.use_save_to_dirs_for_ui:
                namegen = modules.images.FilenameGenerator(
                    p, seed=p.all_seeds[i], prompt=p.all_prompts[i], image=None
                )  # pylint: disable=no-member
                dirname = (
                    namegen.apply(
                        shared.opts.directories_filename_pattern or "[prompt_words]"
                    )
                    .lstrip(" ")
                    .rstrip("\\ /")
                )
    ...
    return gr.File.update(value=fullfns, visible=True), plaintext_to_html(
        f"Saved: {filenames[0] if len(filenames) > 0 else 'none'}"
    )
...
def create_output_panel(tabname, outdir):
    ...
    def open_folder(f):
        if not os.path.exists(f):
            ...
            shared.log.warning(
                f"An open_folder request was made with an argument that is not a folder: {f}"
            )
            return
    ...
    with gr.Column(variant="panel", elem_id=f"{tabname}_results"):
        with gr.Group(elem_id=f"{tabname}_gallery_container"):
            result_gallery = gr.Gallery(
                ...
                preview=False, container=False, columns=[1, 2, 3, 4, 5, 6]
            )  # <576px, <768px, <992px, <1200px, <1400px, >1400px
        ...
        with gr.Column(elem_id=f"{tabname}_footer", elem_classes="gallery_footer"):
            with gr.Row(
                ...
                if not shared.cmd_opts.listen:
                    open_folder_button = gr.Button(
                        ...
                        elem_id=f"open_folder_{tabname}",
                    )
            ...
            with gr.Group():
                html_info = gr.HTML(
                    elem_id=f"html_info_{tabname}",
                    elem_classes="infotext",
                    visible=False,
                )  # contains raw infotext as returned by wrapped call
                html_info_formatted = gr.HTML(
                    elem_id=f"html_info_formatted_{tabname}",
                    elem_classes="infotext",
                    visible=True,
                )  # contains html formatted infotext
                html_info.change(
                    fn=infotext_to_html,
                    inputs=[html_info],
                    outputs=[html_info_formatted],
                    show_progress=False,
                )
                ...
                generation_info_button.click(
                    ...
                    show_progress=False,  # triggered on gallery change from js
                ...
                delete.click(
                    fn=call_queue.wrap_gradio_call(delete_files),
                    _js="(x, y, z, i) => [x, y, z, selected_gallery_index()]",
                    inputs=[generation_info, result_gallery, html_info, html_info],
